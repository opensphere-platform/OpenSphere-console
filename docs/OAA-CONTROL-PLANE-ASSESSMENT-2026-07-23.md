...
> **Current authority notice (2026-07-30):** 이 문서는 구현 이력과 증거를 보존하는 assessment이며 관측 소유권의 정본이 아니다. 현재 계약은 `arch-002 Six-Layer Service Realization Architecture`를 따른다. HIS는 SRL-L1 host prerequisite, Beszel은 SRL-L3 read-only 진단 경로, Shared Observability와 `ObservabilityBinding` provider는 SRL-L4 Platform Support owner다. 이 문서의 과거 `HIS Observability`, `HIS Binding`, `HIS telemetry` 표기는 현재 권위나 lifecycle owner로 해석해서는 안 된다.
...
- `opensphere-his-binding-controller`: 이름은 legacy이나 Cluster Manager가 패키징한 SRL-L4 Platform Support 최소권한 controller다. live Prometheus query, StatefulSet/Deployment readiness와 synthetic canary를 검증해 cluster-scoped `ObservabilityBinding`을 지속 발급·갱신한다. HIS Ready를 대신 판정하지 않는다.
