# OSAA Manual Knowledge Data Model

Status: implementation contract v0.8
Date: 2026-07-23
Scope: OpenSphere AI Agent(OSAA), OpenSphere manuals, Supabase PostgreSQL + pgvector, Console control plane

Product ownership: Manual UI, canonical Help Center documents, release seed and lifecycle are owned exclusively by `OpenSphere-console`. OSAA Gateway provides the Console-owned durable registry/search execution boundary; it is not a separate Manual product owner. See `MANUAL-OWNERSHIP.md`.
...
The current implementation exposes the OSAA manual knowledge store through `/api/manual/*`,
the top-header Documentation search section, and the Console-native `/manual` page.

## 1. Goal

OSAA must not treat OpenSphere manuals as loose text. A manual is operational knowledge with authority, version, navigation context, product scope, and source evidence.
...
The key decision is this: OSAA should not manage manuals as only vectorized text. It should manage manuals as a small knowledge graph backed by Supabase PostgreSQL rows, with pgvector used as the retrieval index for natural-language lookup.

For the current implementation, `ManualDocument` and `ManualChunk` are stored in the migration-owned `osaa` schema:

- `osaa_knowledge_documents`
- `osaa_knowledge_chunks`
...
`ManualDocument` maps to one row in `osaa_knowledge_documents`.
...
| `concept:opensphere:perspective:ai-level` | `perspective` | OSAA, AI gateway, knowledge and automation model |
...
Note: if OpenSphere formally treats this as exactly 10 perspectives, the source manual must resolve whether `website` is part of the ten or a separate public-surface perspective. OSAA should not guess; it should follow the tier-0 or tier-1 manual source.
...
| From | Relation | To |
|---|---|---|
| `concept:opensphere:perspective:ai-level` | `documented-in` | `manual-section:console-docs/osaa-manual-knowledge-data-model#goal` |
| `concept:opensphere:service:osaa-gateway` | `belongs-to` | `concept:opensphere:perspective:ai-level` |
| `concept:opensphere:service:osaa-gateway` | `implemented-by` | `tool:osaa.environment.read` |
| `manual-action:opensphere:osaa-gateway-restart` | `requires-permission` | `role:opensphere-admins` |
...
```text
manual-section:manual:console-docs:platform-control-plane-v2#osaa
```

## 6. ManualChunk

`ManualChunk` maps to one row in `osaa_knowledge_chunks`.
...
`osaa_knowledge_chunks` is a content-addressed serving projection. A document refresh writes
...
OSAA answer generation must receive both chunk text and citation metadata.
...
OSAA should answer using this rule:
...
For operational questions, OSAA must treat manuals and live cluster inspection as different evidence classes.

Operational questions include installation, preflight, plugin lifecycle, Kubernetes runtime state, Data & Identity, Change Control, Foundation, Samba-AD, OpenSearch, and OSAA Gateway troubleshooting.

OSAA must answer in four parts when the question asks what is happening now or what an admin should do:
...
OSAA must not infer current namespaces, pods, services, deployments, CRDs, readiness, install state, or action results from manuals alone. If live inspection is unavailable or incomplete, OSAA must say exactly which fact was not verified.

OSAA may draft a Claim, Binding, manifest, command, or action proposal, but must not say it was applied unless an explicit action endpoint result is present in the conversation. Manual evidence gives permission to propose an action; it does not prove the action already happened.
...
- Keycloak namespace, service name, and current readiness must be read from the live cluster before OSAA mentions them as current facts.
...
Manual knowledge must not stop at explanation. Some manual sections describe actions that OSAA can perform through OpenSphere APIs, CLI commands, Kubernetes APIs, SQL, or UI automation. Those sections need an explicit binding between the manual text and the allowed control surface.

This binding is intentionally separate from `ManualChunk`. A chunk tells OSAA what is true. An action binding tells OSAA what it may do, which tool performs it, which permission is required, and when human confirmation is mandatory.
...
interface ManualActionBinding {
  ...
  intent:
    ...
    | 'ingest-knowledge';
  toolId: string;
  controlPlane: 'opensphere-api' | 'osaa-gateway' | 'kubernetes-api' | 'opensphere-cli' | 'sql' | 'ui';
...
|---|---|---|---|---|
| Read environment snapshot | `osaa.environment.read` | `osaa-gateway` | `read` | `none` |
| Inspect pods in namespace | `osaa.k8s.pods.list` | `kubernetes-api` | `read` | `none` |
| Read recent logs | `osaa.k8s.logs.tail` | `kubernetes-api` | `read` | `none` |
| Query redacted centralized logs | `osaa.observability.logs.query` | `cluster-manager-his-owner-facade` | `read` | `none` |
| Query sanitized distributed traces | `osaa.observability.traces.query` | `cluster-manager-his-owner-facade` | `read` | `none` |
| Restart deployment | `osaa.k8s.deployment.restart` | `kubernetes-api` | `medium` | `required` |
| Scale deployment | `osaa.k8s.deployment.scale` | `kubernetes-api` | `medium` | `required` |
| Inspect an allowlisted Kubernetes resource | `osaa.k8s.resources.list` / `osaa.k8s.resource.get` | `kubernetes-api` | `read` | `none` |
| Update a workload image by immutable digest | `osaa.k8s.workload.update-image` | `kubernetes-api` | `high` | `required` |
| Apply an allowlisted desired-state manifest | `osaa.k8s.resource.apply` | `kubernetes-api` | `high` | `required` |
| Protected resource deletion | `osaa.k8s.resource.delete` | `kubernetes-api` | `critical` | `required` |
| Run or suspend a CronJob | `osaa.k8s.cronjob.run` / `osaa.k8s.cronjob.suspend` | `kubernetes-api` | `high` | `required` |
| Inspect all control-plane owners | `osaa.control-plane.status` | `opensphere-api` | `read` | `none` |
| Search canonical catalog topology | `osaa.catalog.entities.list` | `opensphere-api` | `read` | `none` |
| Read Foundation owner state | `osaa.foundation.status` | `foundation-owner-facade` | `read` | `none` |
| Enable/disable a Foundation engine | `osaa.foundation.engine.lifecycle` | `foundation-owner-facade` | `critical` | `required` |
| Create/release a parameter-free Foundation claim | `osaa.foundation.claim.create` / `osaa.foundation.claim.release` | `foundation-owner-facade` | `high/critical` | `required` |
| Create/release a typed Samba-AD directory claim | `osaa.foundation.identity-directory.claim.create` / `osaa.foundation.identity-directory.claim.release` | `foundation-owner-facade` | `high/critical` | `required` |
| Read sanitized Console users and roles | `osaa.identity.status` | `console-identity-owner-facade` | `read` | `none` |
| Create/enable/disable a Console user | `osaa.identity.user.create` / `osaa.identity.user.enabled` | `console-identity-owner-facade` | `critical` | `required` |
| Add/remove a canonical Console role | `osaa.identity.role.membership` | `console-identity-owner-facade` | `critical` | `required` |
| Read Extension revocations / inspect exact digest | `osaa.extension.security.status` / `osaa.extension.image.inspect` | `dupa-extension-security-owner-facade` | `read` | `none` |
| Revoke an exact-digest Extension image | `osaa.extension.image.revoke` | `dupa-extension-security-owner-facade` | `critical` | `required` |
| Read sanitized Notification operation state | `osaa.notification.status` | `console-notification-owner-facade` | `read` | `none` |
| Enable/disable/test a configured channel | `osaa.notification.channel.enabled` / `osaa.notification.channel.test` | `console-notification-owner-facade` | `critical` | `required` |
| Retry a failed Notification delivery | `osaa.notification.delivery.retry` | `console-notification-owner-facade` | `critical` | `required` |
| Read/plan HISS Observability configuration | `osaa.his.observability.config` / `osaa.his.observability.plan` | `cluster-manager-his-owner-facade` | `read` | `none` |
| Apply HISS Observability configuration | `osaa.his.observability.configure` | `cluster-manager-his-owner-facade` | `critical` | `required` |
| Read/plan external Ceph from a staged import | `osaa.ceph.status` / `osaa.ceph.plan` | `cluster-manager-ceph-owner-facade` | `read` | `none` |
| Connect/disconnect external Ceph | `osaa.ceph.connect` / `osaa.ceph.disconnect` | `cluster-manager-ceph-owner-facade` | `critical` | `required` |
| Read correlated agent evidence | `osaa.evidence.status` | `osaa-supabase-evidence-owner` | `read` | `none` |
| Update retention/legal hold policy | `osaa.evidence.retention.update` | `osaa-supabase-evidence-owner` | `critical` | `required` |
...
OSAA must follow this execution rule:
...
Action bindings reference tools by `toolId`. The tool registry is the machine-readable catalog of what OSAA can actually do in the current OpenSphere environment.

```ts
interface OsaaToolCapability {
  id: string;
  name: string;
  description: string;
  version: string;
...
- `osaa.environment.read`
- `osaa.control-plane.status`
- `osaa.catalog.entities.list`
- `osaa.foundation.status`
...
- `osaa.identity.status`
- `osaa.identity.user.create`
- `osaa.identity.user.enabled`
- `osaa.identity.role.membership`
- `osaa.extension.security.status`
- `osaa.extension.image.inspect`
- `osaa.extension.image.revoke`
- `osaa.notification.status`
- `osaa.notification.channel.enabled`
- `osaa.notification.channel.test`
- `osaa.notification.delivery.retry`
- `osaa.evidence.status`
- `osaa.evidence.retention.update`
- `osaa.k8s.pods.list`
- `osaa.k8s.services.list`
- `osaa.k8s.events.list`
- `osaa.k8s.logs.tail`
- `osaa.k8s.resource.describe`
- `osaa.k8s.deployment.rollout`
- `osaa.k8s.deployment.restart`
- `osaa.k8s.deployment.scale`
- `osaa.k8s.resources.list`
- `osaa.k8s.resource.get`
- `osaa.k8s.workload.restart`
- `osaa.k8s.workload.scale`
- `osaa.k8s.workload.update-image`
- `osaa.k8s.workload.rollback-image`
- `osaa.k8s.resource.apply`
- `osaa.k8s.resource.delete`
- `osaa.k8s.cronjob.run`
- `osaa.k8s.cronjob.suspend`
- `osaa.knowledge.search`
- `osaa.knowledge.ingest-manual`

## 10. Normalized Storage Target

Implemented by `backend/supabase/migrations/0005_osaa_governed_agent.sql` in the `osaa` schema. The production table names are `document`, `document_version`, `section`, `embedding_model`, `embedding`, `concept`, `concept_relation`, `tool_capability`, and `action_binding`, with `osaa_knowledge_documents` and `osaa_knowledge_chunks` retained as the serving contract.

The SQL below is a historical logical-model sketch, not a second schema to create. New code must use migration `0005` rather than these obsolete `osaa_manual_*` names.

```sql
CREATE TABLE osaa_manual_sources (
  id text PRIMARY KEY,
  type text NOT NULL,
  name text NOT NULL,
  base_path text,
...
CREATE TABLE osaa_manual_sections (
  id text PRIMARY KEY,
...
CREATE TABLE osaa_manual_concepts (
  id text PRIMARY KEY,
...
CREATE TABLE osaa_manual_relations (
  id text PRIMARY KEY,
  namespace text NOT NULL,
...
CREATE TABLE osaa_tool_capabilities (
  id text PRIMARY KEY,
  name text NOT NULL,
...
CREATE TABLE osaa_manual_action_bindings (
  id text PRIMARY KEY,
  source_id text NOT NULL,
  section_id text,
  tool_id text NOT NULL REFERENCES osaa_tool_capabilities(id),
  ...
  "documents": [
    {
      ...
      "component": ["supabase", "gitea", "osaa-gateway"],
      "tags": ["platform-control-plane", "supabase", "gitea", "osaa"],
      "content": "..."
    }
  ],
  "concepts": [
    {
      ...
      "authorityTier": 1,
      "status": "active",
      "sourceIds": ["console-docs/osaa-manual-knowledge-data-model"],
      "sectionIds": ["manual-section:console-docs/osaa-manual-knowledge-data-model#goal"],
      "tags": ["osaa", "ai", "perspective"]
    }
  ],
  "relations": [
    {
      "id": "relation:opensphere:osaa-gateway:belongs-to:ai-level",
      "fromId": "concept:opensphere:service:osaa-gateway",
      "relation": "belongs-to",
...
```json
{
  "actionBindings": [
    {
      "id": "manual-action:platform-control:restart-osaa-gateway",
      "sourceId": "console-docs/platform-control-plane-v2",
      "sectionId": "manual-section:console-docs/platform-control-plane-v2#osaa",
      "intent": "restart",
...
Console 관리 > OSAA > Knowledge Store should evolve from a free text form into three modes:
...
The UI should also show action-capable manual sections separately from passive reference material. A human operator must be able to see which manual section gives OSAA permission to propose a write action.
...
OSAA is a Console operator tool, not a generic chat panel. Every retrieval is filtered by the caller's current Console permissions and document ACL before vector or lexical ranking. Supabase records the selected `document_id`, `chunk_id`, and `document_revision` in append-only `osaa.retrieval_trace`; the answer must cite that evidence rather than claim ungrounded system knowledge.

For provider-backed agent runs, the Gateway creates `agent_run.id` before retrieval. `retrieval_trace.agent_run_id`, `tool_run.agent_run_id`, and `llm_usage_event.agent_run_id` correlate the immutable knowledge revision, read-tool evidence, and provider usage with that run. Prompts, responses, credentials, and raw logs are excluded; only digests and bounded metadata are retained. The Admin Agent Evidence view and `osaa.evidence.status` expose this correlation to authorized administrators.

Retention is also typed evidence data, not a hidden maintenance constant. `evidence_retention_policy` defines 30–3650 days, `retain|export-before-delete`, and legal hold for six evidence streams. Every policy change creates an append-only `evidence_policy_event`. OSAA does not expose a purge endpoint: an expired row remains until a reviewed owner maintenance flow has an `evidence_export_receipt` covering it.
...
1. Apply Supabase migrations through `0022_osaa_recovery_owner_permissions.sql` and verify the `osaa` schema, immutable knowledge revisions, append-only runtime events, replica-aware watch cursors, owner projection indexes/comments, correlated agent/retrieval/tool/provider evidence, retention/legal-hold policies, Extension/Notification/HISS/Ceph/Recovery owner permissions, `vector` extension, RLS policies, and constrained `opensphere_osaa_gateway` role.
