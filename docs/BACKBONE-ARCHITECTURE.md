# Console Backbone Service Stack (CBS) — 구현·운영 아키텍처

> **상태**: L1/L2 구현·운영 문서 · **작성일**: 2026-06-29 · **명명 정합일**: 2026-07-12 · **대상 독자**: 콘솔/플랫폼 팀
> **상위 정본**: `../../_DOCS_/01-CONSTITUTION/CONSTITUTION-0004-PLATFORM-BOOTSTRAP-SUPPORT-FOUNDATION-LIFECYCLE.md`
> **범위**: Console Backbone Service Stack이 사용하는 PostgreSQL·RustFS·Gitea의 연결·설치·운영·백업. CBS의 명칭·경계·Ready 의미는 CONSTITUTION-0004가 소유한다.
...
**Console Backbone Service Stack (CBS)** = **Console/Admin 제어·관리 평면 자신**이 동작·확장하기 위해 필요한 자기운영·내구성 Service Stack. `Backbone`은 namespace·CRD·코드에서 사용하는 기술 alias다.

> PFS : 사원·고객·업무 domain service = **CBS : Console/Admin 자신**
>
> **필수 계층**: `HIS preflight → CBS → Main Shell/Console 기능 → subShell → plugin`. CBS는 Console 내부에서 선택 설치하는 기능이 아니며, 세 기둥이 준비되지 않은 Console은 `Not Ready`다.

- **PFS** (`opensphere-foundation`) — 사원·고객·업무 domain service를 떠받치는 공유 capability.
- **CBS** (`opensphere-backbone`) — Console 제어평면을 떠받치는 자원. PFS workload와 **공유되지 않는** Console 전용 상태.

> 표기 규약: 산문에서는 처음에 `Console Backbone Service Stack (CBS)`, 이후 `CBS`로 쓴다. `Backbone` 단독 표기는 namespace, `BackboneClaim`, label과 코드 식별자에만 허용한다. 네임스페이스 `opensphere-backbone`, 라벨 `opensphere.io/backbone-module: <postgres|rustfs|gitea>`.

### 0.2 구성요소

| 컴포넌트 | 역할 | 이미지(핀) |
|---|---|---|
| **PostgreSQL 19 Beta 1 + pgvector 0.8.5** | 콘솔 앱 DB(append-only 감사로그·사용자 설정·플러그인 메타) + Gitea DB 호스팅 | `opensphere-backbone-postgres@sha256:562d7d…` |
| **RustFS** | S3 호환 오브젝트 스토리지(플러그인 번들·Gitea LFS·업로드·백업) | `opensphere-backbone-rustfs@sha256:ae1673…` |
| **Gitea** | 설정 이력 관리(config-as-code / GitOps) | `opensphere-backbone-gitea@sha256:a01aa3…` |
...
> **해소됨(2026-07-11)**: `dupa-audit-log` ConfigMap 폴백을 제거했다. 영구 정본은 PostgreSQL `audit_log`뿐이며, 연결 불가 시 관리 쓰기와 IGA 쓰기는 `503`으로 fail-closed 한다. 메모리 링과 stdout은 조회 캐시·운영 로그일 뿐 감사 증거 저장소가 아니다.
...
> **순환 의존 해소**: Backbone은 Console보다 먼저 독립 부트스트랩된다. Console은 Backbone 없을 때 읽기 전용 복구 표면만 유지할 수 있지만 readiness는 실패하고, 감사/설정 쓰기는 `503`으로 차단한다(§3.5).
...
구현 스크립트 **`tools/local-dev/install-backbone.sh`** 및 Windows용 **`install-backbone.ps1`** (멱등):
...
3. `kubectl apply -f OpenSphere-console/backend/backbone/bootstrap/backbone.yaml` + `rollout status` 대기.
...
- **명시적 degradation**: 세 기둥 중 하나라도 불가하면 Main Shell `/readyz`는 503이다. 읽기 전용 복구 화면은 유지할 수 있으나 관리/IGA 쓰기는 영구 감사 선기록 없이는 실행하지 않는다. Backbone은 Console과 독립 기동한다.
...
- 공식 명칭 **Console Backbone Service Stack (CBS)**, 기술 alias `Backbone`, ns `opensphere-backbone`, 라벨 `opensphere.io/backbone-module`.
...
2. **PostgreSQL 19 GA 전환** — 신규 Setup stable은 현재 `19beta1-alpine` digest로 고정한다. GA 발행 후 새 OpenSphere patch release에서 PostgreSQL 19 GA digest로 교체하며 기존 실행 중인 CBS는 Setup이 변경하지 않는다.
