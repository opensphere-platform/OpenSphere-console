...
- OCI 기준 화면 밀도에 맞춘 검색·상태·범위·JTI·생성·만료·최근 사용 데이터 그리드 제공
- 현재 Console 세션의 Kanidm OIDC PKCE 방식, 만료, 현재 탭 sessionStorage 보관, 내보내기 금지 상태 표시
- 기본 Main Shell이 직접 발급하지 않는 서비스 비밀·OAuth 클라이언트는 가짜 제어 없이 Extension 제공자 미설치 상태로 명시
...
| Edge 이미지 게시 | 9/9 성공, GitHub Actions run `29295112016` |
| Setup 업그레이드 | `sha256:87170f54...`에서 `sha256:c31d0f03...`로 성공 |
...
| Browser E2E | My Profile 검색·empty state·복원·토큰 1회 발급·폐기, 사용자별 토큰 통제 정상 |
| 반응형 QA | 2032×1608, 1024×900, 390×844 통과; body overflow·헤더 충돌·긴 이름 잘림 없음 |
...
현재 edge Console은 Backbone을 필수 기반으로 기동하며, 웹 Console과 `os`가 하나의 신원·권한·API·감사 체계를 공유한다. 사람은 최초 장치 승인 이후 반복 토큰 발급 없이 CLI를 계속 사용할 수 있고, 자동화 토큰은 My Profile의 완성된 자격 증명 화면에서 직접 검색·발급·폐기한다. 관리자는 사용자별 토큰의 안전한 메타데이터와 최근 사용을 확인하고 사유를 남겨 강제 폐기할 수 있지만 원문 조회나 대리 발급은 할 수 없다. Setup이 릴리스 선택부터 digest 고정 설치·검증·롤백·CLI 설치까지 책임지므로 현재 배포는 사후 수동 수정이 아닌 재현 가능한 제품 설치 결과다.
