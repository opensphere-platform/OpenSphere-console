...
test('Console API registry activation rejects every expansion of the approved six-Secret authority', async () => {
  ...
  const mutations = [
    ...
    docs => {docs.find(d=>d.kind==='Deployment').spec.template.spec.volumes.find(v=>v.name==='registry-kubernetes-identity').projected.sources[0].serviceAccountToken.expirationSeconds=86400;},
    docs => {docs.find(d=>d.kind==='Deployment').spec.template.spec.containers[0].volumeMounts.find(v=>v.name==='registry-kubernetes-identity').subPath='token';},
    docs => {docs.find(d=>d.kind==='Deployment').spec.template.spec.volumes.find(v=>v.name==='opensphere-shell-console-api-tls').secret.secretName='other-tls';},
    ...
    docs => {const p=docs.find(d=>d.kind==='NetworkPolicy'); p.spec.egress=p.spec.egress.filter(r=>!r.to?.some(peer=>peer.podSelector?.matchLabels?.app==='opensphere-shell-api'));},
    docs => {docs.find(d=>d.kind==='NetworkPolicy').spec.egress.find(r=>r.to?.some(peer=>peer.podSelector?.matchLabels?.app==='opensphere-shell-api')).to[0].namespaceSelector={};},
    docs => {docs.find(d=>d.kind==='NetworkPolicy').spec.egress.find(r=>r.to?.some(peer=>peer.podSelector?.matchLabels?.app==='opensphere-shell-api')).ports[0].port=8443;},
