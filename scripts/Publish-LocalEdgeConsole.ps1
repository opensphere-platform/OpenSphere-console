...
if ($dockerOs -ne 'linux' -or $dockerArch -notin @('amd64', 'x86_64')) {
  throw "Console edge publishing requires Linux containers on amd64; received $dockerOs/$dockerArch"
}
...
if (((Invoke-Checked git -C $repoRoot branch --show-current) -join '').Trim() -cne 'main') {
  throw 'Console edge publishing runs only from canonical main.'
}
...
$baseRevision = [string]$installedLock.components.console.sourceRevision
if ($baseRevision -notmatch '^[a-f0-9]{40}$') { throw 'Installed Console source revision is not canonical.' }
...
$outputRootBase = Join-Path $platformRoot ".codex-tmp\console-edge-$($sourceRevision.Substring(0,12))"
...
try {
  ...
  if ($digest -notmatch '^sha256:[a-f0-9]{64}$') { throw 'Console build did not produce an exact digest.' }
  ...
  $publication = [ordered]@{
    ...
    channel = 'edge'
    status = 'Active'
    requestIntent = 'Publish the Console UI component required by the current local-edge release.'
  ...
  $evidencePath = Join-Path $outputRoot 'opensphere-console-publication.json'
  $publication | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath $evidencePath -Encoding utf8
  Write-Host '[success] Console-only local edge publication completed'
  ...
  if ($resolvedBuildRoot.StartsWith($tempPrefix, [StringComparison]::OrdinalIgnoreCase) -and
      (Split-Path $resolvedBuildRoot -Leaf) -like 'opensphere-console-edge-*' -and
