...
Contract status is explicit in `contract-denominator.json`. The current denominator contains 19 operations, 5 governed action policies, 11 JSON Schemas, and 10 Console component boundaries. `foundational-slice` means implementation may begin on those operations; it does not claim full API coverage.

`browser-api-cutover.json` is the migration denominator for the actual Console Web source. It assigns every distinct `/api` route pattern to one target component and records which browser-session capabilities remain unavailable. The verifier currently closes 121 route patterns across 15 functional families. Authenticated routes stay on the legacy session authority until every target session capability is implemented and verified; public Registry and CLI artifact routes may remain independently target-routed.
