...
Contract status is explicit in `contract-denominator.json`. The current denominator contains 28 operations, 5 governed action policies, 22 JSON Schemas, and 10 Console component boundaries. `foundational-slice` means implementation may begin on those operations; it does not claim full API coverage.

`browser-api-cutover.json` is the migration denominator for the actual Console Web source. It assigns every distinct `/api` route pattern to one target component and records which browser-session capabilities remain unavailable. The verifier currently closes 122 route patterns across 15 functional families; 12 of 13 target browser-session capabilities are implemented and verified. Authenticated routes stay on the legacy session authority until all 13 capabilities are complete; public Registry and CLI artifact routes may remain independently target-routed.
