...
test('only the initial closed management scenarios map to durable operations', () => {
  assert.deepEqual(durableBindingRequest({ toolId: 'oaa.k8s.workload.restart' },
    { namespace: 'opensphere-console', name: 'console' }),
  { action: 'restart-workload', target: { namespace: 'opensphere-console', name: 'console' } });
  assert.deepEqual(durableBindingRequest({ toolId: 'oaa.notification.delivery.retry' }, { deliveryId: 'delivery-1' }),
    { action: 'retry-delivery', target: { deliveryId: 'delivery-1' } });
  assert.equal(durableBindingRequest({ toolId: 'oaa.k8s.resource.delete' }, {}), null);
});
