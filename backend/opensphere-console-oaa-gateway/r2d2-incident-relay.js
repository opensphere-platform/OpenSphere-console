...
function projectNotification(outbox) {
  ...
  return {
    ...
    route: `/manage/oaa?incident=${encodeURIComponent(outbox.incident_id)}`,
...
class PgIncidentOutboxSource {
  ...
  async claim(workerId, limit) {
    const result = await this.pool.query('SELECT * FROM oaa.claim_incident_outbox($1,$2)', [workerId, limit]);
    return result.rows;
  }
  async delivered(outboxId, workerId) {
    const result = await this.pool.query(`UPDATE oaa.incident_outbox SET status='delivered',delivered_at=clock_timestamp(),
  ...
  async failed(outboxId, workerId, code, terminal) {
    await this.pool.query(`UPDATE oaa.incident_outbox SET status=$3,last_error_code=$4,
