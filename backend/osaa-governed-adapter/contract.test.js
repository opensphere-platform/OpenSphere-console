...
test('canonical declaration digest is deterministic', () => {
  ...
  const b = { inputs: { namespace: 'opensphere-console', name: 'demo' }, toolId: 'osaa.k8s.deployment.restart' };
...
test('governed resource contract is allowlisted, digest pinned, and secret-free', () => {
  ...
  assert.match(resourcePath('deployment', 'opensphere-console', 'demo', { fieldManager: 'osaa' }), /fieldManager=osaa/);
...
test('agent automatic loop exposes reads while mutations remain governed', () => {
  ...
  assert.match(gateway, /const OSAA_MUTATION_NAMESPACES/);
  assert.match(gateway, /mutationNamespaces: OSAA_MUTATION_NAMESPACES/);
  assert.match(gateway, /namespaceScope: OSAA_MUTATION_NAMESPACES/);
  ...
  assert.match(gateway, /'osaa\.control-plane\.status': 'console\.git\.change'/);
...
test('owner lifecycle actions use fixed typed facades with fail-closed inputs and AAL2', () => {
  ...
  assert.match(ownerAction, /OSAA_HIS_VALIDATION_IDS/);
  assert.match(ownerAction, /OSAA_HIS_MANAGED_IDS/);
  assert.match(ownerAction, /OSAA_EXTENSION_LIFECYCLE_ACTIONS/);
  ...
  assert.match(ownerAction, /\/api\/foundation\/osaa\/claims\/create/);
  assert.match(ownerAction, /\/api\/foundation\/osaa\/claims\/release/);
  assert.match(ownerAction, /\/api\/foundation\/osaa\/identity-directory\/claims\/create/);
  assert.match(ownerAction, /\/api\/foundation\/osaa\/identity-directory\/claims\/release/);
  assert.match(ownerAction, /\/api\/osaa\/owner\/identity\/actions/);
  assert.match(ownerAction, /OSAA_FOUNDATION_ENGINES/);
  assert.match(ownerAction, /OSAA_FOUNDATION_MODELS/);
  assert.match(ownerAction, /OSAA_CONSOLE_ROLES/);
  assert.match(ownerAction, /UUID_RE/);
  assert.doesNotMatch(ownerAction, /inputs\.(?:url|path|endpoint|baseUrl)/);
  assert.match(gateway, /OSAA_OWNER_ACTION_TOOL_IDS\.has\(binding\.toolId\)/);
  assert.match(gateway, /allowHisRecovery: \['osaa\.his\.validate', 'osaa\.his\.lifecycle', 'osaa\.his\.observability\.configure'\]\.includes\(binding\.toolId\)/);
  assert.match(gateway, /options\.allowHisRecovery === true && lifecycle\.clusterManagerActivated/);
  assert.match(gateway, /allowCephRecovery: \['osaa\.ceph\.connect', 'osaa\.ceph\.disconnect'\]\.includes\(binding\.toolId\)/);
  ...
  assert.match(gateway, /options\.allowConsoleRecovery === true && OSAA_ACTION_SUBMISSION_ENABLED/);
  ...
  const backendAdapter = backend.slice(backend.indexOf('const OSAA_ACTION_POLICY'), backend.indexOf('async function requireSupabase'));
  assert.doesNotMatch(backendAdapter, /osaa\.(?:his|ceph|extension|platform\.readiness)/);
});
...
test('Console identity owner tools use a fixed PII-minimized facade and independent recovery gate', () => {
  ...
  assert.match(gateway, /'osaa\.identity\.status': 'console\.identity\.manage'/);
  assert.match(gateway, /get_console_identity_status/);
  assert.match(gateway, /\/api\/osaa\/tools\/identity\/status/);
  assert.match(gateway, /backendGet\('\/api\/osaa\/owner\/identity\/status', actor\)/);
  assert.match(gateway, /allowConsoleRecovery: \['osaa\.identity\.user\.create', 'osaa\.identity\.user\.enabled', 'osaa\.identity\.role\.membership'\]\.includes\(binding\.toolId\)/);
  const ownerAction = gateway.slice(gateway.indexOf('async function executeOwnerControlAction'), gateway.indexOf('async function settledControlPlaneComponent'));
...
test('Extension security and Notification operations use typed owner facades without credential inputs', () => {
  ...
  assert.match(gateway, /OSAA_EXTENSION_IMAGE_RE/);
  ...
  assert.match(gateway, /\/api\/osaa\/owner\/extensions\/inspect/);
  assert.match(gateway, /\/api\/osaa\/owner\/extensions\/revoke/);
  assert.match(gateway, /\/api\/osaa\/owner\/notifications\/actions/);
  assert.match(gateway, /allowExtensionSecurity: binding\.toolId === 'osaa\.extension\.image\.revoke'/);
...
test('manual ingestion executes in its Gateway owner boundary and never enters the Kubernetes reconciler', () => {
  ...
  assert.match(executeBinding, /binding\.toolId === 'osaa\.knowledge\.ingest-manual'/);
  ...
  assert.doesNotMatch(backend.slice(backend.indexOf('const OSAA_ACTION_POLICY'), backend.indexOf('async function requireSupabase')), /osaa\.knowledge\.ingest-manual/);
  assert.doesNotMatch(serverSource, /osaa\.knowledge\.ingest-manual/);
});
...
test('DeepSeek DSML tool calls are normalized into the governed tool loop', () => {
  const dsml = '<｜｜DSML｜｜tool_calls>'
    ...
    + '<｜｜DSML｜｜parameter name="query" string="true">OSAA Core &amp; Gateway</｜｜DSML｜｜parameter>'
    + '</｜｜DSML｜｜invoke>'
    + '</｜｜DSML｜｜tool_calls>';
  ...
  assert.deepEqual(JSON.parse(parsed.toolCalls[0].function.arguments), { limit: 8, query: 'OSAA Core & Gateway' });
...
test('outbox claiming and evidence ledgers are durable and least privilege', () => {
  ...
  assert.match(gateway, /OSAA_WATCH_OBSERVER_ID/);
...
test('knowledge refresh preserves historical retrieval evidence by revision', () => {
  ...
  assert.doesNotMatch(gateway, /DELETE FROM osaa_knowledge_chunks/);
});
...
test('agent runs correlate retrieval, tool, and provider evidence with governed retention', () => {
  ...
  assert.match(gateway, /osaa\.evidence\.retention\.update/);
  assert.match(gateway, /evidence retention update requires MFA assurance aal2/);
  assert.match(gateway, /\/api\/osaa\/admin\/evidence/);
});
