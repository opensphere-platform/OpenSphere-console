...
const GITEA_PATH = String(process.env.GITEA_PATH || 'osaa').replace(/^\/+|\/+$/g, '');
const GITEA_TOKEN = process.env.GITEA_TOKEN || '';
const RECONCILER_TOKEN = process.env.RECONCILER_TOKEN || '';
const RECONCILER_NAME = process.env.RECONCILER_NAME || 'osaa-governed-adapter';
...
function resourceTarget(desiredState, allowed = APPLY_KINDS) {
  ...
  const kind = normalizedKind(inputs.kind || (desiredState.toolId?.startsWith('osaa.k8s.deployment.') ? 'deployment' : ''), allowed);
...
async function loadManifest(work) {
  ...
  if (manifest?.metadata?.requestId !== work.request_id || manifest?.metadata?.consumerId !== 'osaa-gateway') throw new Error('governed manifest identity mismatch');
...
async function patchWorkload(manifest, mode) {
  ...
  if (mode === 'restart') {
    ...
    patch = { spec: { template: { metadata: { annotations: {
      'kubectl.kubernetes.io/restartedAt': now, 'opensphere.io/osaa-restarted-at': now, 'opensphere.io/osaa-request-id': manifest.metadata.requestId,
    } } } } };
  } else if (mode === 'scale') {
    ...
    patch = { spec: { template: { metadata: { annotations: { 'opensphere.io/osaa-request-id': manifest.metadata.requestId } }, spec: { containers: [{ name: container, image }] } } } };
...
async function applyResource(manifest) {
  ...
  approved.metadata.annotations = { ...(approved.metadata.annotations || {}), 'opensphere.io/osaa-request-id': manifest.metadata.requestId, 'opensphere.io/managed-by': RECONCILER_NAME };
...
async function runCronJob(manifest) {
  ...
  const job = {
    ...
    metadata: { name: jobName, namespace, labels: { 'opensphere.io/osaa-source-cronjob': name }, annotations: { 'opensphere.io/osaa-request-id': manifest.metadata.requestId } },
...
async function applyManifest(manifest) {
  ...
  if (toolId === 'osaa.k8s.deployment.scale') {
  ...
  if (toolId === 'osaa.k8s.workload.restart') return patchWorkload(manifest, 'restart');
  if (toolId === 'osaa.k8s.workload.scale') return patchWorkload(manifest, 'scale');
  if (toolId === 'osaa.k8s.workload.update-image') return patchWorkload(manifest, 'update');
  if (toolId === 'osaa.k8s.workload.rollback-image') {
...
async function reconcile(work) {
  ...
  try {
    ...
    catch (receiptError) { console.error('[osaa-reconciler] failure receipt rejected:', receiptError.message || receiptError); }
    console.error('[osaa-reconciler] request failed:', work.request_id, lastError);
...
async function pollLoop() {
  while (!stopping) {
    ...
    catch (error) { lastError = String(error?.message || error).slice(0, 500); console.error('[osaa-reconciler] poll failed:', lastError); }
