...
func main() {
	...
	mux.HandleFunc("GET /v1/status", func(w http.ResponseWriter, _ *http.Request) {
		snapshot, ok := store.Current()
		writeJSON(w, http.StatusOK, map[string]interface{}{"service": "opensphere-registry", "product": "OpenSphere Registry & Catalog Service", "cbssCoreService": true, "version": appVersion, "sourceRevision": sourceRevision, "imageDigest": imageDigest, "ready": ok && !snapshot.Stale, "stale": snapshot.Stale, "revision": snapshot.Revision, "observedAt": snapshot.ObservedAt, "sources": snapshot.Sources, "coverage": snapshot.Inventory.Coverage, "rejected": snapshot.Rejected, "lastError": store.LastError()})
	})
