...
// Kubernetes injects the immutable Service IP into every Pod.  Using it avoids
// turning a transient cluster-DNS failure into a total loss of the control API.
  ...
  try {
    const snapshot = await extensionProjection.persist({
      ...
      registry: {
        ...
        capabilities: [],
        plugins: published.map((plugin) => ({ ...plugin, available: true })),
        templates: [],
      },
