...
test('migration grants Platform Support read/manage to admins and read-only to operators', () => {
  const migration = fs.readFileSync(
    path.join(__dirname, '..', 'supabase', 'migrations', '0036_platform_support_observability_permissions.sql'),
    'utf8',
  );
  assert.match(migration, /'console\.platform\.support\.read', 'medium'/);
  assert.match(migration, /'console\.platform\.support\.manage', 'high'/);
  assert.match(
    migration,
    /permission\.code IN \([\s\S]*'console\.platform\.support\.read'[\s\S]*'console\.platform\.support\.manage'[\s\S]*WHERE role\.code = 'console-admins'/,
  );
  assert.match(
    migration,
    /permission\.code = 'console\.platform\.support\.read'[\s\S]*WHERE role\.code = 'console-operators'/,
  );
  assert.doesNotMatch(
    migration,
    /permission\.code = 'console\.platform\.support\.manage'[\s\S]*WHERE role\.code = 'console-operators'/,
  );
});
