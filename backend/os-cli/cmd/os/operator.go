...
func doctor(cfg Config, args []string, out io.Writer) error {
	...
	checks := []struct {
		...
		{"Shared Observability", join(cfg.ConsoleURL, "/api/admin/observability/status"), "Platform Support ObservabilityBinding이 없다면 NotConfigured가 정상이며, 임의 설치로 우회하지 마세요.", false},
