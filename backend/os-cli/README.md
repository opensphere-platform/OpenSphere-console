...
```bash
docker build -t os-cli:console-native-v0.8.0 backend/os-cli
```

Multi-stage build는 Go 테스트와 현재 `index.json`에 공개된 Linux amd64·Windows amd64
바이너리를 빌드한다. macOS arm64는 GitHub Actions의 **native `macos-14` runner**가
Security.framework와 Keychain round-trip을 별도로 검사하지만, 0.8.0 다운로드 manifest에는
아직 선언하지 않으며 Console 이미지에도 복사하지 않는다.

이 경계는 macOS 개인키가 `/usr/bin/security ... -w <secret>`의 argv에 노출되지 않도록
보장한다. manifest에 없는 과거 macOS 바이너리를 암묵적으로 다시 배포하지 않으며, macOS
배포를 추가할 때는 동일 버전·크기·SHA-256 링크를 먼저 `index.json`에 선언해야 한다.
