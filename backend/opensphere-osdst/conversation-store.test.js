...
test('one conversation serializes turns before provider execution', () => {
  ...
  assert.doesNotMatch(source, /reap_expired_dialogue_turns|recover_dialogue_turn|maintenancePoolProvider/);
...
test('Dialogue State transition and assistant message share one fail-closed transaction', () => {
  ...
  assert.match(transition, /r2d2[.]foundation-postgres-intake\/v1/);
  assert.match(transition, /intent: 'status[.]read'/);
  assert.match(transition, /intent: 'create[.]plan'/);
  assert.match(transition, /intent: 'operation[.]watch'/);
