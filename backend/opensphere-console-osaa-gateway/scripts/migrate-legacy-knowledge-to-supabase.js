...
/*
 * One-way OSAA knowledge cutover helper.
 *
...
const dim = Number(process.env.OSAA_EMBED_DIM || 1536);
const required = ['LEGACY_PG_URL', 'OSAA_TARGET_PG_URL', 'OSAA_EMBED_BASE_URL', 'OSAA_EMBED_API_KEY', 'OSAA_EMBED_MODEL'];
for (const name of required) if (!process.env[name]) throw new Error(`${name} is required`);
if (!Number.isInteger(dim) || dim < 16 || dim > 4096) throw new Error('OSAA_EMBED_DIM must be 16..4096');

const legacy = new Pool({ connectionString: process.env.LEGACY_PG_URL, options: '-c search_path=osaa,public' });
const target = new Pool({ connectionString: process.env.OSAA_TARGET_PG_URL, options: '-c search_path=osaa,extensions,public' });
const sha = (value) => createHash('sha256').update(String(value)).digest('hex');
const vec = (values) => `[${values.join(',')}]`;

async function embed(text) {
  const response = await fetch(`${process.env.OSAA_EMBED_BASE_URL.replace(/\/$/, '')}/embeddings`, {
    method: 'POST',
    headers: { authorization: `Bearer ${process.env.OSAA_EMBED_API_KEY}`, 'content-type': 'application/json' },
    body: JSON.stringify({ model: process.env.OSAA_EMBED_MODEL, input: text, dimensions: dim }),
...
async function migrateDocuments() {
  const docs = await legacy.query(`
    ...
    FROM osaa_knowledge_documents ORDER BY source_type, source_id
  `);
  let chunks = 0;
  for (const doc of docs.rows) {
    const sourceChunks = await legacy.query(`
      SELECT chunk_index, content, metadata FROM osaa_knowledge_chunks WHERE document_id = $1 ORDER BY chunk_index
    `, [doc.id]);
    if (!apply) {
    ...
    try {
      ...
      await client.query(`
        INSERT INTO osaa_knowledge_documents
      ...
      await client.query('DELETE FROM osaa_knowledge_chunks WHERE document_id = $1', [doc.id]);
      for (const chunk of enriched) {
        const metadata = { ...(chunk.metadata || {}), embedding: { mode: 'provider', provider: 'migration', model: process.env.OSAA_EMBED_MODEL } };
        await client.query(`
          INSERT INTO osaa_knowledge_chunks (id, document_id, chunk_index, content, embedding, metadata)
...
(async () => {
  try {
    ...
    const concepts = await copyTable('osaa_manual_concepts', ['id','namespace','type','name','aliases','summary','definition','authority_tier','status','source_ids','section_ids','tags','metadata','created_at','updated_at'], '(id) DO UPDATE SET name=EXCLUDED.name, summary=EXCLUDED.summary, definition=EXCLUDED.definition, metadata=EXCLUDED.metadata, updated_at=EXCLUDED.updated_at');
    const relations = await copyTable('osaa_manual_relations', ['id','namespace','from_id','to_id','relation','confidence','authority_tier','source_id','section_id','metadata','created_at','updated_at'], '(id) DO UPDATE SET metadata=EXCLUDED.metadata, updated_at=EXCLUDED.updated_at');
    const tools = await copyTable('osaa_tool_capabilities', ['id','name','version','channel','read_only','spec','created_at','updated_at'], '(id) DO UPDATE SET name=EXCLUDED.name, version=EXCLUDED.version, channel=EXCLUDED.channel, read_only=EXCLUDED.read_only, spec=EXCLUDED.spec, updated_at=EXCLUDED.updated_at');
    const bindings = await copyTable('osaa_manual_action_bindings', ['id','source_id','section_id','tool_id','intent','risk_level','confirmation','spec','created_at','updated_at'], '(id) DO UPDATE SET source_id=EXCLUDED.source_id, section_id=EXCLUDED.section_id, tool_id=EXCLUDED.tool_id, intent=EXCLUDED.intent, risk_level=EXCLUDED.risk_level, confirmation=EXCLUDED.confirmation, spec=EXCLUDED.spec, updated_at=EXCLUDED.updated_at');
    console.log(JSON.stringify({ mode: apply ? 'applied' : 'dry-run', ...documents, concepts, relations, tools, bindings, embeddingModel: process.env.OSAA_EMBED_MODEL, hashVectorsCopied: false }, null, 2));
  } finally {
    await legacy.end();
    await target.end();
  }
