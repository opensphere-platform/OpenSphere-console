...
-- Tombstones are advanced to the current reconcile session so deletion evidence
-- is fenced and auditable.  They are not part of the observer's live node set,
-- so the completeness barrier must compare only non-deleted snapshot rows with
-- the node IDs supplied by the observer.
...
BEGIN
  ...
  SELECT count(*)::integer, coalesce(string_agg(node_id, E'\n' ORDER BY node_id COLLATE "C"),'')
    ...
    WHERE reconcile_session_id=p_session_id AND cluster_id=session_row.cluster_id
      ...
      AND snapshot_complete=true AND deleted_at IS NULL;
...
COMMENT ON FUNCTION oaa.complete_reconcile_session_v2(uuid,integer,integer,boolean,text,text)
  IS 'Fail-closed reconcile barrier over the observer live node set; fenced tombstones remain deletion evidence but are excluded from completeness count and digest.';
