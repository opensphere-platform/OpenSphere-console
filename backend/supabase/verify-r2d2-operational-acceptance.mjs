...
const require = createRequire(path.join(here, '..', 'opensphere-console-osaa-gateway', 'package.json'));
...
BEGIN
  FOREACH role_name IN ARRAY ARRAY[
    ...
    'opensphere_external_channel_executor','opensphere_notification_dispatcher','opensphere_osaa_gateway'
  ] LOOP
...
async function main() {
  ...
  try {
    ...
    await assert.rejects(() => pool.query(`SELECT osaa.assert_evidence_purge_allowed(
    ...
    const exportProof = await pool.query(`INSERT INTO osaa.evidence_partition_export(
    ...
    await pool.query(`INSERT INTO osaa.evidence_legal_hold(stream,range_start,range_end,reason,created_by)
    ...
    await pool.query(`UPDATE osaa.evidence_legal_hold SET active=false,released_by='acceptance',released_at=clock_timestamp()
      WHERE stream='observation' AND range_start=$1`, [retentionStart]);
    assert.equal((await pool.query(`SELECT osaa.assert_evidence_purge_allowed(
    ...
    await pool.query(`INSERT INTO osaa.observer_fence(cluster_id,fencing_epoch,collector_id,lease_identity,lease_expires_at)
      VALUES('acceptance',1,'observer-v1','lease:1',clock_timestamp()+interval '1 hour');
      INSERT INTO osaa.reconcile_session(reconcile_session_id,cluster_id,source,collector_id,fencing_epoch,collection_epoch,
      ...
      INSERT INTO osaa.source_health(cluster_id,source,epistemic_state,configured,snapshot_complete,last_complete_at,last_received_at,lag_seconds)
      VALUES('acceptance','kubernetes','known',true,true,clock_timestamp(),clock_timestamp(),0);
      INSERT INTO osaa.resource_node(cluster_id,node_id,node_type,canonical_id,authority,authority_uid,display_name,namespace,
      ...
      INSERT INTO osaa.resource_relation(cluster_id,from_node_id,relation_type,to_node_id,authority,confidence,evidence_ref,
    ...
    const graphTimes = await samples(pool, `SELECT node_id,node_type,display_name,namespace,health,epistemic_state
      FROM osaa.resource_node WHERE cluster_id='acceptance' AND deleted_at IS NULL
    ...
    await pool.query(`INSERT INTO osaa.incident(incident_id,cluster_id,fingerprint,incident_type,status,severity,confidence,
      ...
      FROM generate_series(1,10000) g;
      INSERT INTO osaa.incident_outbox(incident_id,transition_sequence,event_type,idempotency_key,payload)
      SELECT incident_id,1,'incident_activated','acceptance:'||incident_id,jsonb_build_object('incidentId',incident_id)
      FROM osaa.incident WHERE cluster_id='acceptance';`);
    const incidentTimes = await samples(pool, `SELECT incident_id,severity,status,updated_at FROM osaa.incident
    ...
    await pool.query(`INSERT INTO osaa.observation(cluster_id,source,source_event_id,subject_node_id,observation_type,severity,
    ...
    const history = await pool.query(`SELECT count(*)::int AS rows,count(DISTINCT received_at::date)::int AS days,
      (SELECT coalesce(sum(pg_total_relation_size(inhrelid)),0)::bigint
         FROM pg_inherits WHERE inhparent='osaa.observation'::regclass) AS bytes
      FROM osaa.observation WHERE cluster_id='acceptance'`);
    ...
    const loadRound = async (count, prefix) => {
      const writer = pool.query(`INSERT INTO osaa.observation(cluster_id,source,source_event_id,subject_node_id,observation_type,severity,
        fact,payload_digest,evidence_ref,collector_id,fencing_epoch,collection_epoch,stream_sequence,source_revision,
        reconcile_session_id,snapshot_complete,observed_at,received_at)
    ...
    const outbox = await pool.query(`SELECT count(*)::int AS total,count(DISTINCT idempotency_key)::int AS unique_keys
      FROM osaa.incident_outbox WHERE idempotency_key LIKE 'acceptance:%'`);
    ...
    const replayPair = await pool.query(`WITH selected AS (
        ...
        FROM osaa.incident WHERE cluster_id='acceptance' ORDER BY incident_id LIMIT 2
      )
      INSERT INTO osaa.incident_outbox(incident_id,transition_sequence,event_type,idempotency_key,payload,created_at)
    ...
    const replayed = await pool.query(`SELECT outbox_id FROM osaa.incident_outbox
    ...
    const outboxTimes = await samples(pool, `SELECT outbox_id FROM osaa.incident_outbox
    ...
    const takeover = await pool.query(`SELECT
      osaa.claim_observer_epoch('acceptance','observer-v2','lease:2',60)::int AS epoch`);
    ...
    await assert.rejects(() => pool.query(`INSERT INTO osaa.resource_node(cluster_id,node_id,node_type,canonical_id,authority,
    ...
    const beforeRestart = await pool.query(`SELECT count(*)::int AS nodes FROM osaa.resource_node WHERE cluster_id='acceptance'`);
    ...
    pool = connection();
    assert.equal((await pool.query(`SELECT count(*)::int AS nodes FROM osaa.resource_node WHERE cluster_id='acceptance'`)).rows[0].nodes,
    ...
    const continuity = await restored.query(`SELECT
      (SELECT count(*) FROM osaa.resource_node WHERE cluster_id='acceptance')::int AS nodes,
      (SELECT count(*) FROM osaa.incident WHERE cluster_id='acceptance')::int AS incidents,
      (SELECT count(*) FROM console.module_operation WHERE idempotency_key='acceptance-operation')::int AS operations,
      (SELECT count(*)-count(DISTINCT fingerprint) FROM osaa.incident WHERE cluster_id='acceptance')::int AS duplicate_incidents,
    ...
    assert.equal((await upgradePool.query(`SELECT to_regclass('osaa.resource_node') IS NOT NULL AS ready`)).rows[0].ready, false);
    ...
    assert.equal((await upgradePool.query(`SELECT to_regclass('osaa.resource_node') IS NOT NULL AS ready`)).rows[0].ready, true);
