...
const osaaCutover = read('migrations', '0063_osaa_canonical_identity_and_conversations.sql');
...
assert.match(installer, /rollout', 'status', 'deployment\/opensphere-supabase-auth', '--timeout=10m'/);
assert.match(installer, /--for=jsonpath=\{\.status\.phase\}=Running'[\s\S]+app=opensphere-supabase-storage/);
assert.doesNotMatch(installer, /rollout', 'status', 'deployment\/opensphere-supabase-storage'[\s\S]+migrate-call\.js/);
...
assert.match(osaaCutover, /ALTER SCHEMA oaa RENAME TO osaa/);
assert.match(osaaCutover, /ALTER ROLE %I RENAME TO %I/);
assert.match(osaaCutover, /SET code = regexp_replace\(code, '\^oaa\\\.', 'osaa\.'\)/);
assert.match(osaaCutover, /CREATE TABLE osaa\.conversation \(/);
assert.match(osaaCutover, /CREATE TABLE osaa\.conversation_message \(/);
assert.match(osaaCutover, /UNIQUE \(conversation_id, turn_request_id, role\)/);
assert.match(osaaCutover, /TO opensphere_osaa_gateway/);
assert.doesNotMatch(osaaCutover, /CREATE (?:SCHEMA|ROLE)[^;]*\boaa\b/i);
assert.doesNotMatch(osaaCutover, /\/api\/oaa|oaa\.chat\.use/);
