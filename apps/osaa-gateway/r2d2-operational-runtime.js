...
class OperationalGraphStore {
  ...
  async claimFence(leaseIdentity, ttlSeconds = 60) {
    const result = await this.pool.query('SELECT osaa.claim_observer_epoch($1,$2,$3,$4) AS epoch', [this.clusterId, this.collectorId, leaseIdentity, ttlSeconds]);
    return Number(result.rows[0]?.epoch || 0);
  }

  async renewFence(epoch, ttlSeconds = 60) {
    const result = await this.pool.query('SELECT osaa.renew_observer_epoch($1,$2,$3,$4) AS renewed', [this.clusterId, this.collectorId, epoch, ttlSeconds]);
    return result.rows[0]?.renewed === true;
  }

  async recordSelfModel(input, observedAt = new Date().toISOString()) {
    const model = selfModel(input);
    await this.pool.query(`INSERT INTO osaa.self_model(
  ...
  async reconcile(inventory, fencingEpoch) {
    ...
    try {
      ...
      await client.query(`INSERT INTO osaa.reconcile_session(
      ...
      for (const node of nodes) {
        await client.query(`INSERT INTO osaa.resource_node(
        ...
        ON CONFLICT(cluster_id,node_id) DO UPDATE SET
          ...
          received_at=clock_timestamp(),expires_at=EXCLUDED.expires_at,revision=osaa.resource_node.revision+1,deleted_at=NULL
        WHERE (EXCLUDED.fencing_epoch,EXCLUDED.collection_epoch,coalesce(EXCLUDED.stream_sequence,-1)) >=
              (osaa.resource_node.fencing_epoch,osaa.resource_node.collection_epoch,coalesce(osaa.resource_node.stream_sequence,-1))`, nodeUpsertParams(node));
        const fact = {
        ...
        await client.query(`INSERT INTO osaa.observation(
      ...
      for (const relation of relations) {
        await client.query(`INSERT INTO osaa.resource_relation(
        ...
        WHERE (EXCLUDED.fencing_epoch,EXCLUDED.collection_epoch) >=
              (osaa.resource_relation.fencing_epoch,osaa.resource_relation.collection_epoch)`, [
      ...
      await client.query(`UPDATE osaa.resource_node SET deleted_at=$4,epistemic_state='unknown',revision=revision+1,
      ...
      if (relationSnapshotComplete) {
        await client.query(`UPDATE osaa.resource_relation SET deleted_at=$4,
      ...
      await client.query(
        'SELECT osaa.complete_reconcile_session_v2($1,$2,$3,$4,$5,$6)',
      ...
      for (const source of inventory.sources || []) {
        ...
        await client.query(`INSERT INTO osaa.source_health(cluster_id,source,epistemic_state,configured,snapshot_complete,
          ...
          snapshot_complete=EXCLUDED.snapshot_complete,last_complete_session_id=CASE WHEN EXCLUDED.snapshot_complete THEN EXCLUDED.last_complete_session_id ELSE osaa.source_health.last_complete_session_id END,
          last_complete_at=CASE WHEN EXCLUDED.snapshot_complete THEN EXCLUDED.last_complete_at ELSE osaa.source_health.last_complete_at END,
  ...
  async correlate(fencingEpoch) {
    ...
    try {
      const [nodeResult, relationResult, sourceResult, currentResult] = await Promise.all([
        client.query("SELECT * FROM osaa.resource_node WHERE cluster_id=$1 AND deleted_at IS NULL", [this.clusterId]),
        client.query("SELECT * FROM osaa.resource_relation WHERE cluster_id=$1 AND deleted_at IS NULL", [this.clusterId]),
        client.query("SELECT * FROM osaa.source_health WHERE cluster_id=$1 AND source='kubernetes'", [this.clusterId]),
        client.query('SELECT * FROM osaa.incident WHERE cluster_id=$1', [this.clusterId]),
      ]);
      ...
      for (const signal of signals) {
        ...
        const result = await client.query(`SELECT * FROM osaa.transition_incident(
        ...
        if (signal.incidentType === 'digest_drift') {
          ...
          if (signal.present) {
            await client.query(`INSERT INTO osaa.mismatch(
            ...
            await client.query(`UPDATE osaa.mismatch SET resolved_at=$4,epistemic_state='known'
        ...
        if (transitioned?.incident_id) {
          for (const impact of impacts) {
            await client.query('SELECT osaa.attach_incident_impact($1,$2,$3,$4,$5,$6,$7,$8)', [
          ...
          const evidence = await client.query(`SELECT observation_id,received_at FROM osaa.observation
          ...
          if (evidence.rows[0]?.observation_id) {
            await client.query('SELECT osaa.attach_incident_evidence($1,$2,$3,$4,$5,$6,0)', [
...
class OperationalQueryService {
  ...
  async status() {
    const [sources, nodes, incidents, fence] = await Promise.all([
      this.pool.query('SELECT * FROM osaa.source_health WHERE cluster_id=$1 ORDER BY source', [this.clusterId]),
      this.pool.query("SELECT count(*)::int AS total,count(*) FILTER(WHERE epistemic_state='known' AND expires_at>clock_timestamp())::int AS fresh,max(observed_at) AS observed_at FROM osaa.resource_node WHERE cluster_id=$1 AND deleted_at IS NULL", [this.clusterId]),
      this.pool.query("SELECT count(*) FILTER(WHERE status IN('detected','active','recovering'))::int AS active,max(CASE severity WHEN 'critical' THEN 4 WHEN 'high' THEN 3 WHEN 'warning' THEN 2 ELSE 1 END)::int AS severity_rank FROM osaa.incident WHERE cluster_id=$1", [this.clusterId]),
      this.pool.query('SELECT fencing_epoch,collector_id,lease_expires_at,heartbeat_at FROM osaa.observer_fence WHERE cluster_id=$1', [this.clusterId]),
    ]);
  ...
  async graph(actor, filters = {}) {
    ...
    const result = await this.pool.query(`SELECT * FROM osaa.resource_node WHERE cluster_id=$1 AND deleted_at IS NULL
    ...
    const relations = nodeIds.length ? await this.pool.query(`SELECT * FROM osaa.resource_relation WHERE cluster_id=$1 AND deleted_at IS NULL AND from_node_id=ANY($2::text[]) AND to_node_id=ANY($2::text[])`, [this.clusterId, nodeIds]) : { rows: [] };
  ...
  async node(nodeId, actor) {
    const row = await this.pool.query('SELECT * FROM osaa.resource_node WHERE cluster_id=$1 AND node_id=$2 AND deleted_at IS NULL', [this.clusterId, nodeId]);
    if (!row.rows[0]) return null;
    const relations = await this.pool.query(`SELECT * FROM osaa.resource_relation
  ...
  async incidents(actor, options = {}) {
    ...
    const rows = await this.pool.query(`SELECT i.*,coalesce(jsonb_agg(DISTINCT jsonb_build_object('nodeId',ip.node_id,'impactType',ip.impact_type,'distance',ip.distance,'confidence',ip.confidence)) FILTER(WHERE ip.node_id IS NOT NULL),'[]'::jsonb) AS impacts
      FROM osaa.incident i LEFT JOIN osaa.incident_impact ip ON ip.incident_id=i.incident_id
  ...
  async incident(id, actor) {
    const incident = await this.pool.query('SELECT * FROM osaa.incident WHERE incident_id=$1 AND cluster_id=$2', [id, this.clusterId]);
    if (!incident.rows[0]) return null;
    const [timeline, evidence, impacts] = await Promise.all([
      this.pool.query('SELECT * FROM osaa.incident_timeline WHERE incident_id=$1 ORDER BY sequence', [id]),
      this.pool.query(`SELECT ie.evidence_role,o.observation_id,o.observation_type,o.severity,o.fact,o.evidence_ref,o.observed_at FROM osaa.incident_evidence ie JOIN osaa.observation o ON o.observation_id=ie.observation_id AND o.received_at=ie.observation_received_at WHERE ie.incident_id=$1 ORDER BY ie.rank`, [id]),
      this.pool.query('SELECT * FROM osaa.incident_impact WHERE incident_id=$1 ORDER BY distance,node_id', [id]),
    ]);
  ...
  async metacognition(options = {}) {
    ...
    const [model, mismatches, remediations] = await Promise.all([
      this.pool.query('SELECT * FROM osaa.self_model WHERE cluster_id=$1', [this.clusterId]),
      this.pool.query(`SELECT m.*,a.assessment_id,a.minimum_ladder_step,a.lower_steps,
        ...
        FROM osaa.mismatch m
        LEFT JOIN LATERAL (
          SELECT * FROM osaa.remediation_assessment ra WHERE ra.mismatch_id=m.mismatch_id
      ...
      this.pool.query(`SELECT er.*,be.build_evidence_id,be.source_revision,be.image_digests,
        ...
        FROM osaa.engineering_remediation_request er
        JOIN osaa.remediation_assessment ra ON ra.assessment_id=er.assessment_id
        JOIN osaa.mismatch m ON m.mismatch_id=ra.mismatch_id
        LEFT JOIN LATERAL (SELECT * FROM osaa.build_evidence b WHERE b.remediation_request_id=er.remediation_request_id ORDER BY b.created_at DESC LIMIT 1) be ON true
        LEFT JOIN LATERAL (SELECT * FROM osaa.deployment_verification d WHERE d.remediation_request_id=er.remediation_request_id ORDER BY d.verified_at DESC LIMIT 1) dv ON true
