...
function pkg(overrides = {}) {
  return {
    ...
    metadata: {
      name: 'metrics', resourceVersion: '10', generation: 2,
...
function registration(overrides = {}) {
  return {
    ...
    metadata: { name: 'metrics', uid: 'reg-1', resourceVersion: '11', generation: 3 },
    spec: {
    ...
    status: {
      ...
      currentVersion: '202609020001', currentCompatibilityVersion: '1.2.3',
...
function binding(overrides = {}) {
  return {
    ...
    kind: 'CLIDownload',
    metadata: { name: 'workforce-cli', resourceVersion: '4' },
    spec: {
...
test('management projections are bounded to canonical Package, Registration, preference, and Binding fields', async () => {
  ...
  const fetchImpl = async (url, options) => {
    ...
    if (path.endsWith('/clidownloads')) return json({ items: [binding(), binding({ metadata: { name: 'os', resourceVersion: '1' } })] });
    return json({}, 404);
  };
...
test('desired-state mutation is resourceVersion-bound and rejects shell-pinned removal before write', async () => {
  ...
  const fetchImpl = async (url, options) => {
    ...
    if (options.method === 'PATCH' && path.endsWith('/uipluginregistrations/metrics')) {
      return json(registration({
        metadata: { name: 'metrics', uid: 'reg-1', resourceVersion: '12', generation: 4 },
  ...
  const core = authority(async (url, options) => {
    ...
    if (pathOf(url).endsWith('/uipluginpackages/metrics')) {
      return json(pkg({ metadata: { name: 'metrics', resourceVersion: '10', generation: 2, labels: { 'opensphere.io/scope': 'main-shell-core' } } }));
    }
    return json(registration());
  });
...
test('rollback applies only a verified prior release and marks a second-write failure as present side effect', async () => {
  const prior = {
    ...
    previousVersion: '202608310001',
    previousCompatibilityVersion: '1.1.0',
    ...
    previousEvidenceRefs: ['old-manifest', 'old-signature'],
  ...
  const fetchImpl = async (url, options) => {
    ...
    if (options.method === 'PATCH' && path.endsWith('/uipluginpackages/metrics')) {
      return json(pkg({
        metadata: { name: 'metrics', resourceVersion: '13', generation: 3, labels: { 'opensphere.io/scope': 'workspace-extension' } },
        spec: { ...pkg().spec, ...body.spec, image: { ...pkg().spec.image, ...body.spec.image }, manifest: { ...pkg().spec.manifest, ...body.spec.manifest } },
      }));
    }
    if (options.method === 'PATCH' && path.endsWith('/uipluginregistrations/metrics')) {
      return json(registration({
        metadata: { name: 'metrics', uid: 'reg-1', resourceVersion: '14', generation: 4 },
...
test('Binding mutation uses a compare-and-set resourceVersion and verifies the returned state', async () => {
  ...
  const target = authority(async (url, options) => {
    ...
    return json(binding({ metadata: { name: 'workforce-cli', resourceVersion: '5' }, spec: { ...binding().spec, enabled: false } }));
  });
