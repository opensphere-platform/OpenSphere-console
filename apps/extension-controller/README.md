...
Activation waits until every written ServiceAccount, Deployment, revision Service, and PodDisruptionBudget is re-read and contains the desired managed contract, then requires the exact Deployment rollout. It loads `opensphere-extension-trusted-keys`, hashes the raw manifest bytes, verifies the P-256 signature, hashes the served entry and auxiliary assets within a 16 MiB aggregate asset budget, and rejects artifacts with external module dependencies. Only then may the stable Service move to the verified revision. A changed verified release moves the former `status.current*` coordinates to `status.previous*` in the same status patch; the same digest leaves previous rollback evidence unchanged. Rollback preserves the former repository, manifest paths, key, build provenance, and artifact version, and proceeds only when the current Package still has the same canonical non-coordinate workload and manifest contract digest.
...
The target implements bounded catalog, registration, binding, audit-event, navigation, icon, enable/disable, uninstall, and rollback management routes. Mutations use resourceVersion-fenced Kubernetes writes and durable target audit operations. Browser cutover remains inactive until C_API admission, Nginx routing, database migration, Setup-owned CRD/trust projection, and the approved namespace RBAC are applied together.
