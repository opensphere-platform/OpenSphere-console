...
var commandDefinitions = []commandDefinition{
	...
	{Name: "doctor", Summary: "인증·Registry·Supabase·Gitea·HISS·CRD 진단", Usage: []string{"os doctor [--strict]"}, Options: []string{"--strict"}},
	...
	{Name: "observability", Summary: "Beszel baseline 상태·target 조회", Usage: []string{"os observability status|targets"}},
	{Name: "audit", Summary: "감사 이벤트 조회", Usage: []string{"os audit list"}},
	{Name: "extensions", Summary: "확장 모듈 설치·작업 상태와 Binding 관리", Usage: []string{"os extensions inspect <module-id>", "os extensions install <module-id> --reason TEXT", "os extensions operation get|watch <operation-uuid> [--timeout 5m]", "  예: os extensions install cluster-manager --reason '클러스터 관리 시작'", "  공식 image:channel 또는 image@sha256 입력은 현재 Registry 후보와 정확히 일치할 때만 허용", "os extensions activate|disable|uninstall|rollback|list ...", "os extensions bindings list|enable|disable ...", "os extensions registry status|login|logout ...", "os extensions revocations|revoke-image ..."}, Options: []string{"--reason TEXT", "--timeout DURATION", "--username NAME", "--token-stdin", "--replacement IMAGE"}},
...
var nativeOptionRules = map[string]map[string]bool{
	...
	"extensions operation get":    {},
	"extensions operation watch":  {"timeout": true},
	"extensions activate":         {},
	"extensions disable":          {},
...
var nativeArityRules = map[string]commandArity{
	...
	"observability": {0, 1}, "observability status": {0, 0}, "observability targets": {0, 0},
	...
	"extensions operation": {1, 1}, "extensions operation get": {1, 1}, "extensions operation watch": {1, 1},
