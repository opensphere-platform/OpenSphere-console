...
test('우측 패널 폭 조절 손잡이는 드래그·터치·키보드 조작과 폭 기억을 제공한다', () => {
  assert.match(osPanelTs, /role="separator"/);
  assert.match(osPanelTs, /aria-orientation="vertical"/);
  assert.match(osPanelTs, /tabindex="0"/);
  assert.match(osPanelTs, /\(pointerdown\)="startResize\(\$event\)"/);
  assert.match(osPanelTs, /\(keydown\)="resizeWithKeyboard\(\$event\)"/);
  assert.match(osPanelTs, /document\.addEventListener\('pointermove'/);
  assert.match(osPanelTs, /ev\.key === 'ArrowLeft'/);
  assert.match(osPanelTs, /ev\.key === 'ArrowRight'/);
  assert.match(osPanelTs, /sessionStorage\.setItem\('os-panel-width'/);
  assert.match(
    osPanelTs,
    /\.os-panel-grip::after\s*\{[\s\S]*height:\s*64px;/,
