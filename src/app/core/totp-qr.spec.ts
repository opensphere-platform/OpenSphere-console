...
test('GoTrue v2.189 XML and SVGo comment prefix becomes an image data URL', () => {
  const svg = [
    '<?xml version="1.0"?>',
    '<!-- Generated by SVGo -->',
    '<svg xmlns="http://www.w3.org/2000/svg"><path d="M0 0"/></svg>',
  ].join('\n');
  const result = normalizeTotpQrCode(svg);
  assert.match(result, /^data:image\/svg\+xml;charset=utf-8,/);
  assert.equal(decodeURIComponent(result.split(',', 2)[1]), svg);
});
...
test('unrelated and executable URL schemes are rejected', () => {
  assert.equal(normalizeTotpQrCode('<?xml version="1.0"?><!-- note --><html></html>'), '');
