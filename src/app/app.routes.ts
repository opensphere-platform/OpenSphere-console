...
export const routes: Routes = [
  ...
  // §3.2 Core≠Plugin: 셸 네이티브 컴포넌트. 백엔드는 Console Backend(`/api/identity` 프록시).
  {
    ...
    children: [
      ...
      { path: 'roles', component: AdminRoles },
      {
        ...
        loadComponent: () => import('./pages/admin-foundation-services').then((module) => module.AdminFoundationServices),
      },
      // Compatibility path for the CC experiment terminology. The full
      // Console keeps role-oriented naming and does not reduce its scope to a
  ...
  // Preserve old bookmarks while making /pfss/opensearch the canonical route.
