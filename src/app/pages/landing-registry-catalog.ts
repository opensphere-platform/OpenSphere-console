...
@Component({
  ...
  template: `
    <article class="registry-doc">
      ...
      <section class="flow" aria-labelledby="registry-flow-title">
        <header><p class="eyebrow">DECLARATION TO OPERATION</p><h2 id="registry-flow-title">정의·선택·배포·운영의 권위를 섞지 않습니다.</h2></header>
        <div class="flow-grid">
          <article><span>01</span><img src="/assets/pictograms/systems.svg" alt="Declared sources" width="58" height="58" /><h3>Sources</h3><p>Extension Package·Registration, Foundation capability·offering·plan·runtime catalog·module descriptor를 관측합니다.</p><small>권위: 각 CR Owner</small></article>
  ...
  `,
  styles: [`
    :host{display:block}.registry-doc{--body:.9rem;--detail:.8rem;display:grid;gap:1.35rem;color:var(--os-ink)}
    ...
    .flow{padding:1.3rem;border:1px solid var(--os-hairline);background:var(--os-canvas)}.flow header{margin-bottom:1rem}.flow h2,.contract-grid h2{margin:.3rem 0;font-size:1.15rem}.flow-grid{display:grid;grid-template-columns:repeat(3,minmax(2rem,auto) minmax(0,1fr)) minmax(0,1fr);align-items:stretch}.flow-grid article{display:grid;grid-template-columns:auto 3.7rem minmax(0,1fr);gap:.35rem .7rem;align-content:start;padding:1rem;border:1px solid var(--os-hairline)}.flow-grid article>span{color:var(--os-accent);font:700 .68rem var(--os-font-mono)}.flow-grid article>img{grid-row:1/4}.flow-grid h3{margin:0;font-size:.92rem}.flow-grid p{grid-column:3;margin:.25rem 0;color:var(--os-ink-muted);font-size:var(--detail);line-height:1.55}.flow-grid small{grid-column:3;color:var(--os-ink-subtle);font-size:.7rem}.flow-grid>b{display:grid;place-items:center;padding:.4rem;color:var(--os-accent);font-size:2rem}
