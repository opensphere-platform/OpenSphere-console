...
@Component({
  ...
  template: `
    <main><section><p class="eyebrow">OPENSPHERE CONSOLE</p><h1>{{ auth.mfaRequired() ? '추가 인증' : '로그인' }}</h1>
      ...
      @if (!auth.mfaRequired()) {
        ...
        <form (ngSubmit)="submitMfa()">
          <label>인증 코드<input name="totp" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" maxlength="6" [(ngModel)]="totp" required autofocus></label>
