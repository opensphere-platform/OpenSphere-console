  ...
  template: `
    <div class="os-page">
      ...
      @if (down(); as detail) {
        ...
        <p class="os-sub">
          관측성의 소유자는 <strong>HIS</strong>입니다. Console은 HIS가 발급한
          <code>ObservabilityBinding</code>만 읽어 지표·SLO·경보 표시를 활성화합니다.
        </p>
        ...
        @if (current.mode === 'Connected') {
          <clr-alert clrAlertType="success" [clrAlertClosable]="false"
            ><clr-alert-item
              ><span class="alert-text"
                >HIS Binding 연결됨 — 승인된 관측 데이터만 Console에 표시할 수 있습니다.</span
              ></clr-alert-item
            ></clr-alert
          >
          ...
          <clr-alert clrAlertType="info" [clrAlertClosable]="false"
            ><clr-alert-item
              ><span class="alert-text"
                >HIS Binding이 아직 없습니다. Console은 아래 직접 증거만 표시하며 메트릭·SLO·경보는
                표시하지 않습니다.</span
              ></clr-alert-item
            ></clr-alert
          >
          ...
          <clr-alert clrAlertType="warning" [clrAlertClosable]="false"
            ><clr-alert-item
              ><span class="alert-text"
                >HIS Binding이 준비 중입니다. 완료 전까지 telemetry-dependent 운영 게이트는 충족되지
                않습니다.</span
              ></clr-alert-item
            ></clr-alert
          >
          ...
          <clr-alert clrAlertType="danger" [clrAlertClosable]="false"
            ><clr-alert-item
              ><span class="alert-text"
                >HIS Binding이 저하되었습니다. 마지막 관측 계약을 신뢰 가능한 Ready 상태로 표시하지
                않습니다.</span
              ></clr-alert-item
            ></clr-alert
          >
        ...
        <section class="manage-status-rail" aria-label="Observability 계약 상태">
          <div>
            <span>Authority owner</span><strong>{{ current.owner }}</strong
            ><small>Console은 read-only consumer</small>
          </div>
          <div>
            <span>Binding</span
            ><strong
              [class.ok]="current.mode === 'Connected'"
              [class.warn]="current.mode === 'Pending' || current.mode === 'Degraded'"
              [class.neutral]="current.mode === 'NotConfigured'"
              >{{ current.mode }}</strong
            ><small>{{ current.bindingApi }}</small>
          </div>
          <div>
            <span>Telemetry</span
            ><strong
              [class.ok]="current.telemetry.enabled"
              [class.neutral]="!current.telemetry.enabled"
              >{{ current.telemetry.enabled ? 'Enabled' : 'Not enabled' }}</strong
            ><small>{{ current.telemetry.source }}</small>
          </div>
          <div>
            <span>Capabilities</span
            ><strong>{{ current.capabilities.length }}/{{ capabilityList.length }}</strong
            ><small>HIS 계약상 제공</small>
          </div>
          <div>
            <span>Direct evidence</span
            ><strong>{{ readyEvidence(current) }}/{{ current.directEvidence.length }}</strong
            ><small>Console 가용성만 확인</small>
          </div>
        </section>
          ...
          @if (current.binding; as binding) {
            <dl class="binding-grid">
              <div>
                <dt>이름</dt>
                <dd>
                  <code>{{ binding.namespace }}/{{ binding.name }}</code>
                </dd>
              </div>
              <div>
                <dt>HIS 상태</dt>
                <dd>
                  <span
                    class="label"
                    [class.label-success]="current.ready"
                    [class.label-warning]="!current.ready"
                    >{{ binding.phase }}</span
                  >
                </dd>
              </div>
              <div>
                <dt>관측 시각</dt>
                <dd>{{ binding.observedAt || 'HIS가 아직 보고하지 않음' }}</dd>
              </div>
              <div>
                <dt>승인된 질의 템플릿</dt>
                <dd>{{ binding.templates.length ? binding.templates.join(', ') : '없음' }}</dd>
              </div>
            </dl>
          } @else {
            <p class="empty">
              Binding 없음 — HIS 또는 Cluster Manager에서 Console 소비자용 Binding을 발급해야
              합니다.
            </p>
          }
        ...
        <section class="os-card">
          ...
          <div class="capabilities">
            @for (capability of capabilityList; track capability) {
              <span
                class="label"
                [class.label-success]="hasCapability(current, capability)"
                [class.label-warning]="!hasCapability(current, capability)"
              >
          ...
          <p class="os-sub">
            이 목록은 HIS Binding의 명시적 capability만 반영합니다. Console이 Prometheus/Grafana
            상태를 추측하거나 직접 발견하지 않습니다.
          </p>
        </section>
        ...
        <section class="os-card">
          ...
          <p class="os-sub">
            이 증거는 Console의 기본 가용성 확인용이며, 관측 telemetry 또는 HIS 준비 완료를 대체하지
            않습니다.
          </p>
          <clr-datagrid>
            ...
            @for (item of current.directEvidence; track item.key) {
              <clr-dg-row>
                <clr-dg-cell
                  ><strong>{{ item.label }}</strong></clr-dg-cell
                >
                <clr-dg-cell
                  ><code>{{ item.namespace }}</code></clr-dg-cell
                >
                <clr-dg-cell
                  ><span
                    class="label"
                    [class.label-success]="item.ready"
                    [class.label-warning]="!item.ready"
                    >{{ item.ready ? 'Ready' : 'Unavailable' }}</span
                  ></clr-dg-cell
                >
...
export class AdminObservability implements OnInit, OnDestroy {
  ...
  readyEvidence(state: ObservabilityState): number {
    return state.directEvidence.filter((item) => item.ready).length;
  }

  async refresh(silent = false): Promise<void> {
    if (!silent) this.busy.set(true);
    try {
      const response = await this.http.request('/api/admin/observability/status', {
        cache: 'no-store',
      });
      if (!response.ok) {
        this.state.set(null);
        this.down.set(`HIS Binding status HTTP ${response.status}`);
        return;
      }
