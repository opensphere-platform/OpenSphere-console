        ...
        @if (activeTab() === 'integrations') {
          <section class="domain-workspace" role="tabpanel" aria-label="Integrations">
              <os-datagrid [columns]="integrationColumns" [rows]="current.integrations" empty="등록된 Console consumer contract가 없습니다.">
                ...
                <ng-template osCell="binding" let-item><span class="label" [class.label-success]="item.observability?.phase === 'Bound'" [class.label-warning]="item.observability?.phase !== 'Bound'">{{ item.observability?.phase || 'NotConfigured' }}</span><small>{{ item.observability?.binding || 'HISS Binding 없음' }}</small></ng-template>
              </os-datagrid>
              <p class="os-sub">HISS Binding이 <strong>Bound</strong>일 때만 telemetry freshness를 Console에 표시합니다. Binding이 없으면 Console이 Prometheus를 새로 만들거나 metric을 추정하지 않고 <strong>NotConfigured</strong>으로 반응합니다.</p>
          </section>
        }
      }
    </div>
...
export class AdminDataIdentity implements OnInit, OnDestroy {
  ...
  readonly integrationColumns: OsColumn[] = [{ key: 'consumer', label: 'Consumer' }, { key: 'status', label: '계약 상태' }, { key: 'data', label: 'Supabase 경계' }, { key: 'binding', label: 'HISS Binding' }];
