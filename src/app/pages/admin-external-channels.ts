...
@Component({
  ...
  template: `
    ...
    <os-panel [open]="backupTargetPanelOpen()" [title]="editingBackupTargetId() ? 'S3 백업 대상 편집' : 'S3 백업 대상 추가'" subtitle="Backblaze B2 · 대상별 credentials are write-only" (closed)="closePanels()">
      @if (panelError(); as issue) { <div class="panel-error" role="alert"><strong>{{ fieldLabel(issue.field) }}</strong><span>{{ issue.message }}</span></div> }
      <div class="panel-callout"><strong>{{ editingBackupTargetId() ? '대상 메타데이터를 수정하고 필요할 때 자격 증명을 교체합니다.' : '각 외부 저장소의 실제 Backblaze B2 정보를 입력합니다.' }}</strong><span>Region에 대응하는 HTTPS S3 endpoint만 허용됩니다. 예: https://s3.us-east-005.backblazeb2.com</span><span class="warn">모든 snapshot은 서버 암호화 설정과 관계없이 업로드 전에 AES-256-GCM으로 암호화됩니다.</span></div>
      <form clrForm clrLayout="vertical" class="channel-form" autocomplete="off">
      ...
      <div osPanelFooter><button class="btn btn-primary" [disabled]="busy() || !backupTargetFormValid()" (click)="saveBackupTarget()">{{ editingBackupTargetId() ? '변경 저장' : '대상 추가' }}</button><button class="btn btn-outline" [disabled]="busy()" (click)="closePanels()">취소</button></div>
    </os-panel>
