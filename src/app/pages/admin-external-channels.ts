...
@Component({
  ...
  template: `
    ...
    <os-panel [open]="backupTargetPanelOpen()" title="S3 백업 대상 연결" subtitle="Backblaze B2 · credentials are write-only" (closed)="closePanels()">
      ...
      <form clrForm clrLayout="vertical" class="channel-form" autocomplete="off">
        <clr-input-container><label>연결 이름</label><input clrInput [(ngModel)]="backupTargetForm.name" name="backup-name" minlength="2" maxlength="80" required [attr.aria-invalid]="backupTargetFieldError('name') ? 'true' : null" />@if (backupTargetFieldError('name'); as issue) { <div class="field-error">{{ issue }}</div> }</clr-input-container>
      ...
      <div osPanelFooter><button class="btn btn-primary" [disabled]="busy() || !backupTargetFormValid()" (click)="createBackupTarget()">연결 저장</button><button class="btn btn-outline" [disabled]="busy()" (click)="closePanels()">취소</button></div>
    </os-panel>
...
export class AdminExternalChannels {
  ...
  openBackupTargetPanel(): void { this.panelError.set(null); this.backupTargetForm = emptyBackupTarget(); this.backupTargetPanelOpen.set(true); }
  closePanels(): void { this.panelError.set(null); this.channelPanelOpen.set(false); this.rulePanelOpen.set(false); this.backupTargetPanelOpen.set(false); this.editingChannelId.set(null); }
  async createBackupTarget(): Promise<void> { if (!this.backupTargetFormValid()) return; await this.mutatePanel(async () => { await this.request('/api/external-channels/backup-targets', this.backupTargetForm); this.backupTargetForm.accessKeyId = ''; this.backupTargetForm.applicationKey = ''; this.closePanels(); }); }
  ...
  backupTargetFieldError(field: string): string {
    ...
    if (field === 'accessKeyId' && !/^[A-Za-z0-9]{25}$/.test(value.accessKeyId.trim())) return 'Key Name이나 Bucket ID가 아닌 Backblaze의 25자 keyID를 입력하세요.';
    if (field === 'applicationKey' && !/^[A-Za-z0-9]{31}$/.test(value.applicationKey.trim())) return '키 생성 직후 한 번 표시되는 31자 applicationKey를 입력하세요.';
    return '';
  }
  backupTargetFormValid(): boolean { return ['name', 'bucketName', 'pathPrefix', 'accessKeyId', 'applicationKey'].every((field) => !this.backupTargetFieldError(field)); }
  fieldLabel(field: string): string { return ({ accessKeyId: 'Application Key ID', applicationKey: 'Application Key', bucketName: 'Bucket name', region: 'Region / Endpoint', pathPrefix: 'Object prefix', name: '연결 이름' } as Record<string, string>)[field] || '요청 오류'; }
  ...
  private openPendingAction(title: string, description: string, path: string, confirmLabel: string, testRecipient?: string): void { this.panelError.set(null); this.pendingAction.set({ title, description, path, confirmLabel, reason: '운영자 수동 실행', testRecipient }); }
  cancelPendingAction(): void { this.panelError.set(null); this.pendingAction.set(null); }
  async executePendingAction(): Promise<void> { const action = this.pendingAction(); if (!action) return; await this.mutatePanel(async () => { await this.request(action.path, { reason: action.reason.trim(), ...(action.testRecipient !== undefined ? { testRecipient: action.testRecipient.trim() } : {}) }); this.cancelPendingAction(); }); }
