...
interface ChangeRequest {
  ...
  approvalPolicy?: 'local-edge-automation' | 'owner-mfa' | 'cross-operator';
...
export class AdminPlatformControl implements OnInit, OnDestroy {
  ...
  private buildJourneySteps(change: ChangeRequest | null): JourneyStep[] {
    ...
    const automated = change.approvalPolicy === 'local-edge-automation';
    const approved = automated
      ? Boolean(change.execution?.merge_revision || change.git_commit_sha || change.status === 'committed' || change.status === 'applied')
      : change.approvals.some((item) => item.status === 'applied');
    ...
    return [
      ...
      { column: '4', source: 'Gitea', label: automated ? (approved ? 'Local edge automated' : 'Automation pending') : (change.approvalPolicy === 'owner-mfa' ? (approved ? 'Owner MFA authorized' : 'Awaiting owner MFA') : (approved ? 'Cross approval' : 'Awaiting approval')), evidence: automated ? 'audience-bound ServiceAccount' : (change.approvalPolicy === 'owner-mfa' ? `${change.approvals.length} authorization` : `${change.approvals.length}/${this.gitea()?.supplyChain?.requiredApprovals || 1}`), time: change.approvals.at(-1)?.completed_at || change.execution?.updated_at || null, state: approved ? 'done' : (failed ? 'failed' : 'waiting') },
