...
import { canonicalNavBand } from '../core/perspectives';
...
@Component({
  ...
  template: `
    <div class="os-page">
      <os-page-header title="Console Extensions" tag="Admin Control">
        <p>
          UI 플러그인(UIPluginPackage) + headless 바인딩(CLIDownload) 통합 인식·관리 — 셸 리빌드
          없이
        </p>
      </os-page-header>

      @if (controlPlaneDown(); as detail) {
        <os-backend-unavailable
          feature="Console Extensions 제어"
          backend="opensphere-console-dupa-controller (/api/admin/plugins)"
          hint="서명 Registry의 실행 기능과 Extension 설치·활성화 제어 평면은 별도입니다. DUPA 제어 런타임이 배포되기 전에는 관리 작업을 제공하지 않습니다."
          [detail]="detail"
        />
      }
      ...
      <section class="manage-status-rail" aria-label="Extension 운영 상태">
        <div>
          <span>Catalog</span><strong>{{ catalog().length }}</strong
          ><small>서명된 패키지</small>
        </div>
        <div>
          <span>Active</span><strong class="ok">{{ countPhase('Activated') }}</strong
          ><small>메뉴·workload 활성</small>
        </div>
        <div>
          <span>Ready</span><strong>{{ countPhase('Ready') }}</strong
          ><small>활성화 대기</small>
        </div>
        <div>
          <span>Disabled</span><strong class="neutral">{{ countPhase('Disabled') }}</strong
          ><small>운영 제외</small>
        </div>
        <div>
          <span>Failed</span
          ><strong [class.danger]="countPhase('Failed') > 0">{{ countPhase('Failed') }}</strong
          ><small>검토 필요</small>
        </div>
        <div>
          <span>Bindings</span><strong>{{ bindings().length }}</strong
          ><small>headless channels</small>
        </div>
      </section>
        ...
        <clr-accordion class="management-actions">
          <clr-accordion-panel>
            ...
            <clr-accordion-description
              >설치 · Registry 자격증명 · Digest 철회</clr-accordion-description
            >
            <clr-accordion-content *clrIfExpanded>
              <section class="registry-access" aria-labelledby="registry-access-title">
                <div class="registry-access-head">
                  <div>
                    ...
                    <p class="os-sub">
                      공개 전환 없이 private OpenSphere 패키지를 검사하고 Kubernetes workload가
                      pull하도록 동일한 read-only 자격증명을 사용합니다. 토큰은 화면에 다시 표시되지
                      않습니다.
                    </p>
                  </div>
                  @if (registryStatus(); as registry) {
                    <span class="label" [class.label-success]="registry.configured">{{
                      registry.configured ? 'Configured' : 'Not configured'
                    }}</span>
                  }
                </div>
                <div class="registry-access-form">
                    ...
                    <div class="clr-control-container">
                      <div class="clr-input-wrapper">
                        <input
                          ...
                          placeholder="read:packages 권한 토큰"
                        />
                      </div>
                    </div>
                  </div>
                    ...
                    <div class="clr-control-container">
                      <div class="clr-input-wrapper">
                        <input
                          ...
                          #registryReason
                          class="clr-input"
                          placeholder="등록 또는 제거 승인 사유(8자 이상)"
                        />
                      </div>
                    </div>
                  </div>
                  <button
                    ...
                    [disabled]="
                      registryToken.value.length < 20 || registryReason.value.trim().length < 8
                    "
                    (click)="
                      configureRegistryCredentials(
                        registryUser.value,
                        registryToken.value,
                        registryReason.value
                      );
                      registryToken.value = ''
                    "
                  >
                  ...
                  <button
                    ...
                    [disabled]="
                      !registryStatus()?.configured || registryReason.value.trim().length < 8
                    "
                    (click)="removeRegistryCredentials(registryReason.value)"
                  >
                    제거
                  </button>
                </div>
              ...
              <section class="registry-access" aria-labelledby="revocation-title">
                <div class="registry-access-head">
                  <div>
                    ...
                    <p class="os-sub">
                      취약하거나 손상된 exact digest를 Supabase append-only 원장에 철회합니다.
                      철회는 수정·삭제할 수 없고 신규 설치 및 활성 Registry 투영을 차단합니다.
                    </p>
                  </div>
                ...
                <div class="registry-access-form">
                    ...
                    <div class="clr-control-container">
                      <div class="clr-input-wrapper">
                        <input
                          ...
                          class="clr-input"
                          size="70"
                          placeholder="ghcr.io/opensphere-platform/...@sha256:..."
                        />
                      </div>
                    </div>
                  </div>
                  <div class="clr-form-control">
                    <label for="replacement-image" class="clr-control-label"
                      >Replacement digest (optional)</label
                    >
                    <div class="clr-control-container">
                      <div class="clr-input-wrapper">
                        <input
                          ...
                          class="clr-input"
                          size="52"
                          placeholder="same repository@sha256:..."
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    [disabled]="
                      !revokeImageRef.value.includes('@sha256:') ||
                      revokeReason.value.trim().length < 8
                    "
                    (click)="
                      revokeImage(
                        revokeImageRef.value,
                        replacementImageRef.value,
                        revokeReason.value
                      )
                    "
                  >
                ...
                @if (revocations().length) {
                  <table class="table table-compact">
                    ...
                    <tbody>
                      @for (item of revocations(); track item.repository + item.digest) {
                        <tr>
                          <td class="left os-mono">{{ item.repository }}&#64;{{ item.digest }}</td>
                          <td class="os-mono">{{ item.replacementDigest || '—' }}</td>
                          <td>{{ item.actor }}</td>
                          <td>{{ item.revokedAt }}</td>
                          <td class="left">{{ item.reason }}</td>
                        </tr>
                      }
                    </tbody>
                  </table>
              ...
              <section class="oci-install" aria-labelledby="oci-install-title">
                ...
                <p class="os-sub">
                  OpenSphere GHCR 이미지의 SDK 계약·서명·권한 프로필을 먼저 검증합니다.
                  채널(edge/candidate/stable)은 설치 전에 불변 digest로 해석되며, 메뉴 활성화는 별도
                  승인입니다.
                </p>
                <div class="clr-form-control">
                  ...
                  <div class="clr-control-container">
                    <div class="clr-input-wrapper">
                      <input
                        ...
                        class="clr-input"
                        size="90"
                        placeholder="ghcr.io/opensphere-platform/opensphere-shell-foundation:edge"
                      />
                    </div>
                  </div>
                </div>
                  ...
                  <div class="clr-control-container">
                    <div class="clr-input-wrapper">
                      <input
                        ...
                        class="clr-input"
                        size="60"
                        placeholder="설치 목적과 승인 근거(8자 이상)"
                      />
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  [disabled]="
                    !inspection() || inspection()?.requestedImage !== imageRef.value.trim()
                  "
                  (click)="installImage(imageRef.value, reasonRef.value)"
                >
                  설치
                </button>
                @if (inspection(); as plan) {
                  <div class="inspection-plan" role="status">
                    ...
                    <span class="label label-success"
                      >Descriptor {{ plan.verification.descriptor }}</span
                    >
                    <span class="label label-success"
                      >Signature {{ plan.verification.signature }}</span
                    >
                    <span class="label label-success"
                      >Provenance {{ plan.verification.provenance }}</span
                    >
                    <span class="label label-success">SBOM {{ plan.verification.sbom }}</span>
                    <span class="label">{{ plan.descriptor.permissionProfile }}</span>
                    @if (plan.channel) {
                      <span class="label label-info">{{ plan.channel }} → digest</span>
                    }
                    <span class="os-mono">{{ plan.image }}</span>
                    <span class="os-mono">{{ plan.verification.platforms.join(', ') }}</span>
                    @if (plan.registryCredentialsRequired) {
                      <span class="label label-info">Private pull credential</span>
                    }
                    ...
                    <clr-alert [clrAlertType]="'info'" [clrAlertClosable]="false"
                      ><clr-alert-item
                        ><span class="alert-text"
                          >Foundation은 Ready 상태까지 사전 설치할 수 있습니다. 메뉴 활성화는
                          Platform Support Profile Ready 이후에 허용됩니다.
                          <a routerLink="/manage/platform-control"
                            >Control Plane에서 준비 상태 확인</a
                          ></span
                        ></clr-alert-item
                      ></clr-alert
                    >
            ...
            <div class="tree">
              @for (root of tree(); track root.id) {
                <div class="tn tn0 host">
                  <button class="caret" (click)="toggle(root.id)">
                    {{ exp(root.id) ? '▾' : '▸' }}
                  </button>
                ...
                @if (exp(root.id)) {
                  @for (c of root.children; track c.id) {
                    <div class="tn tn1">
                      @if (c.type === 'subShell') {
                        <button class="caret" (click)="toggle(c.id)">
                          {{ exp(c.id) ? '▾' : '▸' }}
                        </button>
                      } @else {
                        ...
                        <div class="tn tn2">
                          ...
                          @if (g.phase) {
                            <span
                              ...
                              [class.label-danger]="effectiveStateByName(g.id).tone === 'danger'"
                              >{{ effectiveStateByName(g.id).label }}</span
                            >
                          }
            ...
            <table class="table">
              ...
              <tbody>
                @for (r of registrations(); track r.name) {
                  <tr>
                    <td class="left">
                      <button type="button" class="extension-link" (click)="select(r.name)">
                        {{ displayName(r.name) }}
                      </button>
                    ...
                    <td>
                      <span
                        class="label"
                        [class.label-success]="workloadPhase(r) === 'Ready'"
                        [class.label-danger]="
                          workloadPhase(r) === 'Degraded' || workloadPhase(r) === 'NotReady'
                        "
                        >{{ workloadPhase(r) }}</span
                      >
                    </td>
                    <td>
                      <span
                        class="label"
                        [class.label-success]="menuState(r).visible"
                        [class.label-warning]="!menuState(r).visible"
                        >{{ menuState(r).label }}</span
                      >
                    ...
                    <td>
                      <span
                        class="label"
                        [class.label-success]="r.status.channelState === 'Current'"
                        [class.label-danger]="r.status.channelState === 'SecurityActionRequired'"
                        >{{ r.status.currentRequestedChannel || 'exact' }} ·
                        {{ r.status.channelState || '—' }}</span
                      >
                    </td>
                    <td>
                  ...
                  <tr>
                    ...
                    <td>
                      @switch (phaseOf(c.name)) {
                        @case ('Enabled') {
                          <button class="btn btn-sm" (click)="run('disable', c.name)">
                            Disable
                          </button>
                        }
                        @case ('Activated') {
                        ...
                        @case ('Ready') {
                          <button
                            ...
                            [title]="
                              foundationActivationLocked(c.name)
                                ? 'Platform Support Profile Ready 필요'
                                : ''
                            "
              ...
              <tbody>
                @for (b of bindings(); track b.name) {
                  <tr>
                    ...
                    <td>
                      <span class="label label-info">{{ b.kind }}</span>
                    </td>
                    <td>
                    ...
                    <td>
                      @for (l of b.links; track l.href) {
                        <a class="btn btn-sm btn-link" [href]="l.href" target="_blank">{{
                          l.text
                        }}</a>
                      }
                    </td>
                    <td>
                      @if (b.enabled !== false) {
                        <button class="btn btn-sm" (click)="runBinding('disable', b.name)">
                          Disable
                        </button>
                      } @else {
                        <button
        ...
        >
          ...
          @if (!menuState(r).visible) {
            <clr-alert [clrAlertType]="'warning'" [clrAlertClosable]="false">
              <clr-alert-item
                ><span class="alert-text"
                  ><strong>메뉴 미노출</strong> — {{ menuState(r).reason }}. Extension은 실행 중일
                  수 있지만 전역 메뉴에는 표시되지 않습니다.</span
                ></clr-alert-item
              >
            </clr-alert>
          }
          ...
          <div class="cc-primary-actions">
            @if (pageReady(r)) {
              <a class="btn btn-sm btn-primary" [routerLink]="['/p', r.name]"
                >Extension 페이지 열기</a
              >
            }
          ...
          @if (r.status.phase === 'Failed' && r.status.reason) {
            <div class="cc-reason">
              <strong>사유</strong>
              <div>
                {{ reasonText(r.status.reason) }}
                <span class="os-mono">({{ r.status.reason }})</span>
              </div>
            </div>
          }
          ...
          <section class="cc-integrations" aria-labelledby="cc-integrations-title">
            <div class="cc-section-head">
              <div>
                <h3 id="cc-integrations-title">Console 연동 상태</h3>
                <p>메뉴·페이지·API·검색·문서·관측 신호를 각각 확인합니다.</p>
              </div>
                ...
                @for (item of integrationRows(r); track item.key) {
                  <tr>
                    <td class="left">
                      ...
                      <div class="os-mono">{{ item.key }}</div>
                    </td>
                    <td>
                      <span
                        class="label"
                        [class.label-success]="item.status.phase === 'Ready'"
                        [class.label-warning]="
                          item.status.phase === 'Disabled' ||
                          item.status.phase === 'DependencyPending'
                        "
                        [class.label-danger]="
                          item.status.phase === 'Failed' || item.status.phase === 'Degraded'
                        "
                        >{{ integrationPhaseLabel(item.status.phase) }}</span
                      >
                    </td>
                    <td class="left">
                      {{ item.status.reason || item.status.message || '연동 준비 완료' }}
                    </td>
                    <td class="os-mono">{{ item.status.observedVersion || '—' }}</td>
                  </tr>
                } @empty {
                  <tr>
                    <td colspan="4" class="left os-sub">
                      연동 상태가 아직 보고되지 않았습니다. 컨트롤러 상태를 새로고침하세요.
                    </td>
                  </tr>
                }
              </tbody>
          ...
          <clr-accordion class="cc-secondary">
            <clr-accordion-panel>
              ...
              <clr-accordion-description>{{
                r.status.currentVersion || r.status.observedVersion || '버전 미보고'
              }}</clr-accordion-description>
              <clr-accordion-content *clrIfExpanded>
                <div class="cc-steps">
                ...
                <dl class="cc-kv">
                  <dt>등록 phase</dt>
                  <dd>{{ r.status.phase || '—' }}</dd>
                  <dt>워크로드</dt>
                  <dd>{{ workloadPhase(r) }}</dd>
                  <dt>사유</dt>
                  <dd>{{ r.status.reason || '—' }}</dd>
                  <dt>마지막 변경</dt>
                  <dd class="os-mono">{{ r.status.lastTransitionTime || '—' }}</dd>
                  <dt>manifest</dt>
                  <dd class="os-mono cc-break">{{ r.status.manifestUrl || '—' }}</dd>
                  <dt>요청자</dt>
                  <dd>{{ r.approval?.requestedBy || '—' }}</dd>
                  <dt>승인 사유</dt>
                  <dd>{{ r.approval?.reason || '—' }}</dd>
                </dl>
              </clr-accordion-content>
            </clr-accordion-panel>
                  ...
                  <div class="cc-iconpick-note">
                    {{
                      iconLib.list().length
                        ? iconMatchCount() +
                          '개 일치' +
                          (iconMatchCount() > iconList().length
                            ? ' · 상위 ' + iconList().length + '개 표시'
                            : '')
                        : '라이브러리 로딩 중…'
                    }}
                  </div>
                  <div class="cc-iconpick-grid">
                    ...
                    @for (c of iconList(); track c.token) {
                      <button
                        type="button"
                        class="cc-iconbtn"
                        [class.sel]="iconToken() === c.token"
                        [title]="c.label"
                        (click)="chooseIcon(c.token)"
                      >
          ...
          <div class="cc-actions" aria-label="Extension lifecycle actions">
            @if (r.status.phase === 'Activated') {
              ...
              <button class="btn btn-sm btn-success-outline" (click)="run('enable', r.name)">
                Enable (재검증)
              </button>
            }
            <button class="btn btn-sm btn-danger-outline" (click)="run('uninstall', r.name)">
              Uninstall
            </button>
          </div>
          ...
          @if (r.status.phase === 'Failed') {
            <p class="os-sub">
              서명 검증 실패 시 nav에 노출되지 않습니다(보안 게이트). 유효 서명으로 재배포 후
              Enable(재검증)하세요.
            </p>
          }
        </os-panel>
      ...
      <os-action-dialog
        ...
        title="Extension 제거"
        [message]="
          pendingUninstall() ? pendingUninstall() + '의 메뉴와 워크로드를 제거합니다.' : ''
        "
        confirmLabel="제거"
        [danger]="true"
  ...
  styles: [
    `
      ...
      .management-actions .registry-access:first-child {
        margin-top: 0.7rem;
      }
      .oci-install {
        padding: 0.8rem 1rem;
      ...
      .cc-dot {
        ...
        height: 0.6rem;
        border-radius: 50%;
        background: var(--os-ink-subtle);
        flex: 0 0 auto;
      }
      .cc-state-success .cc-dot {
        background: var(--os-success);
      }
      .cc-state-danger .cc-dot {
        background: var(--os-error);
      }
      .cc-state-warning .cc-dot {
        background: var(--os-warning);
      }
      .cc-state-danger {
      ...
      .cc-layer-success {
        border-top-color: var(--os-success);
      }
      .cc-layer-warning {
        border-top-color: var(--os-warning);
      }
      .cc-layer-danger {
        border-top-color: var(--os-error);
      }
      .cc-layer-neutral {
        border-top-color: var(--os-ink-subtle);
      }
      .cc-layer-label {
        color: var(--os-muted);
        font-size: 0.62rem;
      ...
      .cc-step-fail .cc-step-ic {
        color: var(--os-error);
      }
      .cc-step-active .cc-step-ic {
        color: var(--os-accent);
      }
      .cc-step-pending {
        opacity: 0.6;
...
export class AdminPlugins implements OnInit {
  ...
  readonly controlPlaneDown = signal('');
  ...
  selectedPanelSubtitle(): string {
    ...
    const version =
      registration?.status.currentVersion || registration?.status.observedVersion || item?.version;
  ...
  integrationRows(r: Registration): IntegrationRow[] {
    ...
    return Object.entries(r.status.integrations || {})
      ...
      .sort(
        (a, b) =>
          (order.indexOf(a.key) < 0 ? 99 : order.indexOf(a.key)) -
          (order.indexOf(b.key) < 0 ? 99 : order.indexOf(b.key)),
      );
  }
  ...
  integrationSummary(r: Registration): string {
    ...
    const issues = rows.filter((x) =>
      ['Failed', 'Degraded', 'DependencyPending'].includes(x.status.phase),
    ).length;
  ...
  menuState(r: Registration): { visible: boolean; label: string; reason: string } {
    ...
    if (loadedPage) {
      return {
        visible: true,
        label: '메뉴 노출',
        reason: `${canonicalNavBand(loadedPage.navBand)} · ${this.controlCenter.pluginRoute(r.name)}`,
      };
    }
    ...
    if (this.ext.loadState() === 'loading')
      return { visible: false, label: 'Host 적재 중', reason: 'Extension Host가 검증·등록하는 중' };
    if (r.status.phase !== 'Activated')
      return {
        visible: false,
        label: '메뉴 미노출',
        reason: `Registration ${r.status.phase || '미보고'} 상태`,
      };
    if (!this.catalogItem(r.name)?.nav)
      return { visible: false, label: '메뉴 미선언', reason: 'UIPluginPackage spec.nav가 없음' };
    return {
      visible: false,
      label: '메뉴 미노출',
      reason: 'Activated이지만 Extension Host pages registry에 적재되지 않음',
    };
  }
  ...
  effectiveStateByName(name: string): EffectiveExtensionState {
    ...
    return r
      ? this.effectiveState(r)
      : { label: '미설치', detail: 'Registration 없음', tone: 'neutral' };
  }
  effectiveState(r: Registration): EffectiveExtensionState {
    ...
    if (phase === 'Failed' || failed.length) {
      return {
        label: phase === 'Failed' ? '실패' : '연동 저하',
        detail: r.status.reason || failed.map((x) => x.label).join(', '),
        tone: 'danger',
      };
    }
    if (phase === 'Disabled')
      return {
        label: '비활성',
        detail: '워크로드는 유지되지만 Console에서 비활성화됨',
        tone: 'neutral',
      };
    if (!['Activated', 'Ready'].includes(phase)) {
      return {
        label: phase,
        detail: r.status.reason || '설치·검증 진행 상태',
        tone: phase === 'Degraded' ? 'danger' : 'warning',
      };
    }
    ...
    if (pending.length)
      return {
        label: `${phase} · 연동 대기`,
        detail: pending.map((x) => x.label).join(', '),
        tone: 'warning',
      };
    return {
      label: phase === 'Activated' ? 'Activated · 연동 완료' : 'Ready · 활성화 대기',
      detail: this.integrationSummary(r),
      tone: phase === 'Activated' ? 'success' : 'warning',
    };
  }
  statusLayers(r: Registration): StatusLayer[] {
    ...
    const verified =
      ...
      verificationValues.every((v) => v === 'Verified' || v === 'Approved');
    const workload = this.workloadPhase(r);
    const menu = this.menuState(r);
    const integrationIssue = this.integrationRows(r).some((x) =>
      ['Failed', 'Degraded', 'DependencyPending'].includes(x.status.phase),
    );
    return [
      {
        label: '1. Artifact',
        value: verified ? 'Verified' : verificationValues.length ? '확인 필요' : '미보고',
        detail: 'manifest · signature · digest · permission',
        tone: verified ? 'success' : 'warning',
      },
      {
        label: '2. Workload',
        value: workload,
        detail: 'Pod · Service · health',
        tone:
          workload === 'Ready'
            ? 'success'
            : workload === 'Degraded' || workload === 'NotReady'
              ? 'danger'
              : 'warning',
      },
      {
        label: '3. Registration',
        value: r.status.phase || '미보고',
        detail: r.status.reason || 'DUPA lifecycle',
        tone:
          r.status.phase === 'Activated' || r.status.phase === 'Ready'
            ? 'success'
      ...
      {
        label: '4. Console integration',
        value: integrationIssue ? '확인 필요' : this.integrationSummary(r),
        detail: 'page · API · manual · search · observability',
        tone: integrationIssue ? 'danger' : 'success',
      },
      {
        label: '5. User visibility',
        value: menu.label,
        detail: menu.reason,
        tone: menu.visible ? 'success' : 'warning',
      },
    ];
  }
  ...
  reasonText(reason?: string): string {
    const m: Record<string, string> = {
      ...
      PermissionProfileDrift:
        'DUPA가 요구하는 고정 RBAC 권한 프로파일과 설치된 ClusterRole 규칙이 다름',
    };
  ...
  steps(): { label: string; state: 'done' | 'fail' | 'pending' | 'active' }[] {
    ...
    if (phase === 'Ready')
      return this.VSTEPS.map((s, i) => ({
        label: s.label,
        state: (i < this.VSTEPS.length - 1 ? 'done' : 'pending') as any,
      }));
    if (phase === 'Disabled')
      return this.VSTEPS.map((s, i) => ({
        label: s.label,
    ...
    if (phase === 'Failed' && reason) {
      ...
      return this.VSTEPS.map((s, i) => ({
        label: s.label,
        state:
          fi < 0 ? (i === 0 ? 'done' : 'pending') : i < fi ? 'done' : i === fi ? 'fail' : 'pending',
      }));
    }
    // Installing/기타 — 1단계 진행 중
    return this.VSTEPS.map((s, i) => ({
      label: s.label,
      state: (i === 0 ? 'active' : 'pending') as any,
    }));
  }
  ...
  async refresh(): Promise<void> {
    ...
    this.catalog.set(required[0].status === 'fulfilled' ? required[0].value : []);
    this.registrations.set(required[1].status === 'fulfilled' ? required[1].value : []);
    this.events.set(required[2].status === 'fulfilled' ? required[2].value : []);
    this.bindings.set(required[3].status === 'fulfilled' ? required[3].value : []);
    this.foundationActivationAllowed.set(
      optional[0].status === 'fulfilled' &&
        optional[0].value.admission.foundationActivationAllowed === true,
    );
    this.registryStatus.set(optional[1].status === 'fulfilled' ? optional[1].value : null);
    this.revocations.set(optional[2].status === 'fulfilled' ? optional[2].value : []);

    const requiredFailures = required
      .map((result, index) =>
        result.status === 'rejected'
          ? `${['catalog', 'registrations', 'events', 'bindings'][index]}: ${String(result.reason)}`
          : '',
      )
      .filter(Boolean);
    this.controlPlaneDown.set(
      requiredFailures.length === required.length
        ? `Extension 제어 API 4개가 모두 응답하지 않았습니다. ${requiredFailures.join(' · ')}`
        : '',
    );
    const allFailures = [
      ...requiredFailures,
      ...optional
        .map((result, index) =>
          result.status === 'rejected'
            ? `${['readiness', 'registry credentials', 'revocations'][index]}: ${String(result.reason)}`
            : '',
        )
        .filter(Boolean),
    ];
    this.msg.set(
      allFailures.length && !this.controlPlaneDown()
        ? {
            type: 'danger',
            text: `일부 Extension 상태를 불러오지 못했습니다: ${allFailures.join(' · ')}`,
          }
        : null,
    );
  }

  async configureRegistryCredentials(
    username: string,
    token: string,
    reason: string,
  ): Promise<void> {
    try {
      this.registryStatus.set(
        await this.ctl.configureRegistryCredentials(username.trim(), token.trim(), reason.trim()),
      );
      this.msg.set({
        type: 'success',
        text: 'Private GHCR read credential이 저장되었습니다. 토큰 값은 다시 표시되지 않습니다.',
      });
    } catch (err) {
      this.msg.set({ type: 'danger', text: `GHCR 자격증명 저장 실패: ${err}` });
    }
  }
  ...
  async removeRegistryCredentials(reason: string): Promise<void> {
    try {
      ...
      this.msg.set({ type: 'danger', text: `GHCR 자격증명 제거 실패: ${err}` });
    }
  }
  ...
  async revokeImage(image: string, replacementImage: string, reason: string): Promise<void> {
    try {
      ...
      this.msg.set({
        type: 'success',
        text: 'Image digest가 철회되었고 신규 설치 및 활성 Registry 투영이 차단됩니다.',
      });
      await this.refresh();
    } catch (err) {
      this.msg.set({ type: 'danger', text: `Image 철회 실패: ${err}` });
    }
  }
  ...
  async inspectImage(image: string): Promise<void> {
    ...
    try {
      ...
      this.msg.set({ type: 'danger', text: `검증 실패: ${err}` });
    }
  }
  ...
  async installImage(image: string, reason: string): Promise<void> {
    ...
    try {
      ...
      this.msg.set({
        type: 'info',
        text: `설치 요청됨: ${result.id} — 검증 완료 후 Ready 상태가 됩니다.`,
      });
      await this.poll(result.id, 'install');
      await this.refresh();
    } catch (err) {
      this.msg.set({ type: 'danger', text: `설치 실패: ${err}` });
    }
  }
  ...
  private classify(c: CatalogItem): 'core' | 'subShell' | 'plugin' {
    ...
    if (c.core || /admin|main-?shell|console-admin/i.test(c.scope || c.nav?.band || ''))
      return 'core';
  ...
  private buildTree(): TreeNode[] {
    ...
    const mainPlugins = plugins.filter(
      (p) => !p.hostRef || !subs.some((s) => s.name === p.hostRef),
    );
