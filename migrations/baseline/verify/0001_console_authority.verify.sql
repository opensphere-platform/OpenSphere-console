...
BEGIN
  ...
  IF v_status->>'authority' <> 'Supabase'
      ...
      OR v_status->'data'->'components'->4->>'baselineRevision' <> 'opensphere-console/20260903/0028'
      OR v_status->'data'->'components'->4->>'setDigest' <> 'sha256:da5993f76c206dff8a748c912ce91fbb9fb929cb3210f86f1063de29d7eff2d5'
      OR v_status->'data'->'components'->4->>'migrationCount' <> '28'
      OR v_status->'data'->'components'->5->>'state' <> 'Ready'
      OR v_status->'data'->'components'->5->>'protectedTables' <> '16' THEN
    RAISE EXCEPTION 'Supabase status projection overclaimed or lost baseline evidence: %', v_status;
  END IF;
  BEGIN
  ...
  END;
  BEGIN
    PERFORM * FROM console_migration.applied_migration;
    RAISE EXCEPTION 'console_api read migration ledger directly';
  EXCEPTION WHEN insufficient_privilege THEN NULL;
  END;
END;
$$;
...
DO $$
BEGIN
  IF (SELECT count(*) FROM console_migration.applied_migration) <> 28 THEN
    RAISE EXCEPTION 'fresh migration ledger cardinality mismatch';
  END IF;
  BEGIN
    UPDATE console_migration.applied_migration SET migration_set_size = 2;
    RAISE EXCEPTION 'migration ledger update unexpectedly succeeded';
  EXCEPTION WHEN SQLSTATE '55000' THEN NULL;
  END;
  BEGIN
    DELETE FROM console_migration.applied_migration;
    RAISE EXCEPTION 'migration ledger delete unexpectedly succeeded';
  EXCEPTION WHEN SQLSTATE '55000' THEN NULL;
  END;
  BEGIN
    TRUNCATE console_migration.applied_migration;
    RAISE EXCEPTION 'migration ledger truncate unexpectedly succeeded';
  EXCEPTION WHEN SQLSTATE '55000' THEN NULL;
  END;
END;
$$;
...
BEGIN
  ...
  IF (SELECT count(*) FROM pg_class WHERE relrowsecurity AND relnamespace IN (
    ...
    RAISE EXCEPTION 'expected sixteen RLS-protected authority tables';
  END IF;
  IF EXISTS (
    SELECT 1
