-- CON-FR-009: current authority metadata; preserve session and permission admission.
...
BEGIN
  ...
  -- Explicit protection inventory. An extra table cannot silently become Ready.
  WITH expected(name, protection) AS (VALUES
    ('console_identity.subject_authority','Rls'),
    ('console_identity.browser_session','Rls'),
    ('console_identity.permission_grant','Rls'),
    ('console_identity.cli_challenge','Rls'),
    ('console_identity.cli_device','Rls'),
    ('console_identity.cli_enrollment','Rls'),
    ('console_identity.cli_session','Rls'),
    ('console_operation.operation','Rls'),
    ('console_operation.outbox','Rls'),
    ('console_operation.approval','Rls'),
    ('console_operation.verification_receipt','Rls'),
    ('console_operation.execution_receipt','Rls'),
    ('console_extension.registry_connection','Rls'),
    ('console_extension.revocation','Rls'),
    ('console_extension.presentation_preference','Rls'),
    ('console_audit.event','Rls'),
    ('console_operation.module_installation_environment','OwnerOnly')
  ), actual AS (
    SELECT n.nspname||'.'||c.relname AS name, c.* FROM pg_class c
    JOIN pg_namespace n ON n.oid=c.relnamespace
    WHERE n.nspname IN ('console_identity','console_operation','console_audit','console_extension')
      AND c.relkind IN ('r','p')
  ), checks AS (
    SELECT COALESCE(e.name,a.name) AS name, e.protection,
      a.oid IS NOT NULL AS present,
      COALESCE(a.relrowsecurity AND a.relforcerowsecurity,false) AS rls,
      CASE
        WHEN a.oid IS NULL OR e.name IS NULL THEN false
        WHEN e.protection='Rls' THEN a.relrowsecurity AND a.relforcerowsecurity
        WHEN e.protection='OwnerOnly' THEN
          NOT EXISTS (SELECT 1 FROM aclexplode(COALESCE(a.relacl,acldefault('r',a.relowner))) x
            WHERE x.grantee<>a.relowner)
          AND NOT EXISTS (SELECT 1 FROM pg_roles r
            WHERE r.rolname IN ('console_api','console_extension_controller','anon','authenticated','service_role',
              'opensphere_osaa_gateway','opensphere_shell_api','opensphere_console_backend')
              AND has_table_privilege(r.oid,a.oid,'SELECT,INSERT,UPDATE,DELETE,TRUNCATE,REFERENCES,TRIGGER'))
        ELSE false
      END AS protected
    FROM expected e FULL JOIN actual a USING(name)
  )
  SELECT count(*) FILTER(WHERE present)::integer,
    count(*) FILTER(WHERE rls)::integer,
    bool_and(protected),
    jsonb_agg(jsonb_build_object('table',name,'protection',COALESCE(protection,'Unclassified'),
      'state',CASE WHEN protected THEN 'Ready' ELSE 'Blocked' END,
      'reasonCode',CASE WHEN NOT present THEN 'TableMissing' WHEN protection IS NULL THEN 'TableUnclassified'
        WHEN NOT protected AND protection='OwnerOnly' THEN 'RuntimeGrantPresent'
        WHEN NOT protected THEN 'RlsCoverageIncomplete' ELSE NULL END) ORDER BY name)
    INTO v_authority_table_count,v_rls_table_count,v_protection_ready,v_protection
    FROM checks;

  -- Aggregate metadata only: no subject IDs, email, session handles or object names.
  SELECT jsonb_build_object(
    'operators',(SELECT count(*) FROM console_identity.subject_authority),
    'activeSessions',(SELECT count(*) FROM console_identity.browser_session s
      JOIN console_identity.subject_authority a ON a.subject_id=s.subject_id
      WHERE s.revoked_at IS NULL AND s.expires_at>v_observed_at
        AND s.permission_revision=a.permission_revision AND s.revoke_epoch=a.revoke_epoch),
    'auditEvents',(SELECT count(*) FROM console_audit.event),
    'roles',console_identity.managed_role_catalog(),
    'buckets',(SELECT COALESCE(jsonb_agg(jsonb_build_object('id',id,'name',name,'public',public,
      'fileSizeLimit',file_size_limit) ORDER BY id),'[]'::jsonb) FROM storage.buckets),
    'accessBindings',(
      SELECT COALESCE(jsonb_agg(jsonb_build_object('role',role_name,'schema',schema_name,
        'executableFunctions',function_count,'selectableTables',table_count) ORDER BY role_name,schema_name),'[]'::jsonb)
      FROM (
        SELECT r.rolname AS role_name,n.nspname AS schema_name,
          (SELECT count(*) FROM pg_proc p WHERE p.pronamespace=n.oid
            AND has_function_privilege(r.oid,p.oid,'EXECUTE')) AS function_count,
          (SELECT count(*) FROM pg_class c WHERE c.relnamespace=n.oid AND c.relkind IN ('r','p')
            AND has_table_privilege(r.oid,c.oid,'SELECT')) AS table_count
        FROM pg_roles r CROSS JOIN pg_namespace n
        WHERE r.rolname IN ('console_api','console_extension_controller','opensphere_osaa_gateway','opensphere_shell_api')
          AND n.nspname IN ('console_identity','console_operation','console_audit','console_extension','console_shell','osaa')
          AND has_schema_privilege(r.oid,n.oid,'USAGE')
      ) b WHERE function_count>0 OR table_count>0
    )
  ) INTO v_inventory;
  ...
  RETURN jsonb_build_object(
    ...
    'data', jsonb_build_object(
      ...
      'components', jsonb_build_array(
        ...
        jsonb_build_object(
          'component', 'rls',
          'state', CASE WHEN v_protection_ready THEN 'Ready' ELSE 'Blocked' END,
          'authority', 'PostgreSQLCatalog',
          'reasonCode', CASE WHEN v_protection_ready THEN NULL ELSE 'RlsCoverageIncomplete' END,
          'authorityTables', v_authority_table_count,
          'protectedTables', v_rls_table_count,
          'scope', 'console_identity,console_operation,console_audit,console_extension',
          'checks', v_protection
        ),
