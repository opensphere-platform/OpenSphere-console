...
# This deploy worktree depends on the sibling OpenSphere-SDK package.
...
COPY OpenSphere-console-standard-worktree/package.json OpenSphere-console-standard-worktree/package-lock.json ./
RUN npm ci --no-audit --no-fund --legacy-peer-deps
COPY OpenSphere-console-standard-worktree/angular.json OpenSphere-console-standard-worktree/tsconfig.json OpenSphere-console-standard-worktree/tsconfig.app.json OpenSphere-console-standard-worktree/tsconfig.spec.json ./
COPY OpenSphere-console-standard-worktree/scripts ./scripts
COPY OpenSphere-console-standard-worktree/public ./public
COPY OpenSphere-console-standard-worktree/src ./src
...
WORKDIR /src
COPY OpenSphere-console-standard-worktree/backend/os-cli/go.mod ./
COPY OpenSphere-console-standard-worktree/backend/os-cli/cmd ./cmd
...
RUN mkdir -p /out && go test ./... && \
    CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags="-s -w -X main.version=0.5.0" -o /out/opensphere-cli-linux-amd64 ./cmd/os && \
    install -m 0755 /prebuilt/opensphere-cli-darwin-arm64 /out/opensphere-cli-darwin-arm64 && \
    CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -trimpath -ldflags="-s -w -X main.version=0.5.0" -o /out/opensphere-cli-windows-amd64.exe ./cmd/os
...
WORKDIR /manifest
COPY OpenSphere-console-standard-worktree/backend/os-cli/index.json ./index.json
COPY OpenSphere-console-standard-worktree/backend/os-cli/generate-manifest.mjs ./generate-manifest.mjs
...
COPY OpenSphere-console-standard-worktree/nginx/default.conf.template /etc/nginx/templates/default.conf.template
RUN set -eu; \
